...
# ==========================================
# v10 overrides: general editorial engine for daily GR/public-affairs briefing
# ==========================================

V6_VERSION = "google_rss_v10_general_editorial_engine_daily"

# v10 목표: 특정 인물/오늘 사건을 하드코딩하지 않고, 매일 재사용 가능한 사건 유형(signal)으로 선별한다.
# 후보 구성은 랭킹 상위 + 중요 위험 신호 후보 + 카테고리 균형 후보를 혼합한다.
...
# v10: 특정 인물명 중심 shard는 줄이고, 재사용 가능한 위험 신호 기반 shard를 확장한다.
...
# 엔티티 alias는 특정 이슈가 아니라 계열사/서비스 일반 표현을 넓히는 용도.
...
SELF_KAKAO_PATTERN = re.compile(
    r"카카오|카톡|Kakao|카나나|카카오페이|카카오뱅크|카카오모빌리티|카카오\s*T\b|카카오T\b|카카오택시|카카오게임즈|카카오엔터|카카오헬스케어|카카오엔터프라이즈|카카오픽코마|디케이테크인|엑스엘게임즈|AXZ",
    re.IGNORECASE,
)
...
# v10 general signal taxonomy: 매일 반복 사용되는 위험/중요 사건 유형.
V10_SIGNAL_PATTERNS = {
    "leadership_change": re.compile(r"(대표|임원|경영진|CPO|CTO|CFO|C레벨|최고[가-힣A-Za-z]*책임자|성과리더|본부장|리더|책임자).{0,80}(퇴사|사의|사퇴|사임|물러|떠난|교체|내정|선임|영입|해임|직무배제|조직개편|쇄신)|조직개편|리더십\s*공백|컨트롤타워", re.IGNORECASE),
...
EVENT_PATTERNS.update({k: p.pattern for k, p in V10_SIGNAL_PATTERNS.items()})
STRONG_EVENT_TAGS.update(set(V10_SIGNAL_PATTERNS.keys()))

V10_SELF_CRITICAL_SIGNALS = {
    ...
    "financial_performance", "ai_infrastructure_security", "digital_asset_stablecoin",
}
V10_GOV_CRITICAL_SIGNALS = {
    "policy_legislation", "legal_regulatory_enforcement", "privacy_security_reliability",
    "market_power_platform", "ai_infrastructure_security", "digital_asset_stablecoin",
}
V10_COMPETITOR_CRITICAL_SIGNALS = {
    "strategy_mna_governance", "legal_regulatory_enforcement", "privacy_security_reliability",
    "market_power_platform", "ai_infrastructure_security", "digital_asset_stablecoin", "financial_performance",
}
V10_SIGNAL_WEIGHTS = {
    "leadership_change": 28,
    "service_reform_backlash": 25,
    "labor_negotiation": 24,
    "labor_law_complaint": 30,
    "legal_regulatory_enforcement": 26,
    "privacy_security_reliability": 24,
    "policy_legislation": 20,
    "market_power_platform": 22,
    "strategy_mna_governance": 21,
    "financial_performance": 12,
    "ai_infrastructure_security": 18,
    "digital_asset_stablecoin": 18,
}

V10_STORY_TOPIC_LIMITS = {
    ...
    "self:ai_strategy": 2,
    "government:regulatory_enforcement": 3,
    "government:policy_legislation": 3,
    "government:security_policy": 2,
    "government:digital_asset": 2,
    "competitor:strategy_mna": 2,
    "competitor:regulatory_enforcement": 2,
    "competitor:security_reliability": 2,
    "competitor:ai_strategy": 2,
    "industry:ai_infrastructure": 2,
    "industry:digital_asset": 1,
}
V10_CATEGORY_FAMILY_TOTAL_LIMITS = {
    ("자사_및_계열사_이슈", "labor_"): 3,
    ("자사_및_계열사_이슈", "leadership_service"): 2,
    ("자사_및_계열사_이슈", "financial_performance"): 2,
    ("정부_국회", "security_policy"): 2,
    ("정부_국회", "regulatory_enforcement"): 3,
}
V10_MUST_REVIEW_TOPIC_POOL_LIMIT = 14

_BASE_collection_relevance_reason_v10 = collection_relevance_reason
_BASE_article_importance_score_v10 = article_importance_score
_BASE_is_article_obviously_low_value_v10 = is_article_obviously_low_value
_BASE_is_report_item_relevant_v10 = is_report_item_relevant
_BASE_final_duplicate_reason_v10 = final_duplicate_reason
_BASE_event_tag_limit_reason_v10 = event_tag_limit_reason


def v10_detect_signals(text):
    text = clean_html_text(text)
    found = []
    for name, pattern in V10_SIGNAL_PATTERNS.items():
...
def v10_main_entity_group(text, json_key=""):
    text = clean_html_text(text)
    if SELF_KAKAO_PATTERN.search(text) or json_key == "자사_및_계열사_이슈":
        return "self"
...
def v10_story_family_from_signals(signals, text="", json_key=""):
    s = set(signals)
    text = clean_html_text(text)
    group = v10_main_entity_group(text, json_key)
    ...
    if "digital_asset_stablecoin" in s:
        return "digital_asset"
    if "market_power_platform" in s:
        return "platform_market"
    return "general"


def v10_story_topic_bucket(item):
    text = f"{item.get('기사제목','')} {item.get('본문전문','')[:1800]} {item.get('본문요약','')} {item.get('RSS요약','')} {item.get('검색어','')}"
    json_key = item.get("카테고리") or item.get("JSON카테고리") or ""
    signals = v10_detect_signals(text)
    if not signals:
        return ""
    group = v10_main_entity_group(text, json_key)
    family = v10_story_family_from_signals(signals, text, json_key)
    return f"{group}:{family}"


def story_topic_bucket_v9(item):
    # 이전 main/로그 호환용 alias. 실제 동작은 v10 범용 버킷을 사용한다.
    return v10_story_topic_bucket(item)
...
def v10_must_review_reason(text, json_key=""):
    ...
    if not text:
        return False, ""
    if v10_is_individual_crime_case(text, json_key):
        return False, ""
    ...
    if group == "self" and signal_set & V10_SELF_CRITICAL_SIGNALS:
        return True, f"must_review:{group}:{family}:{v10_signal_string(signals)}"
    if group == "government" and signal_set & V10_GOV_CRITICAL_SIGNALS:
        return True, f"must_review:{group}:{family}:{v10_signal_string(signals)}"
    if group == "competitor" and signal_set & V10_COMPETITOR_CRITICAL_SIGNALS:
        return True, f"must_review:{group}:{family}:{v10_signal_string(signals)}"
    if group == "industry" and signal_set & {"ai_infrastructure_security", "digital_asset_stablecoin", "market_power_platform"}:
        return True, f"must_review:{group}:{family}:{v10_signal_string(signals)}"
    return False, ""
...
def collection_relevance_reason(keyword, title, summary):
    text = f"{clean_html_text(title)} {clean_html_text(summary)[:900]} {keyword}"
    must, _reason = v10_must_review_reason(text, CATEGORY_TO_JSON_KEY.get(keyword_to_category.get(keyword, ""), ""))
    if must:
        return ""
    return _BASE_collection_relevance_reason_v10(keyword, title, summary)


def article_importance_score(title, body="", json_key="", keyword=""):
    base = float(_BASE_article_importance_score_v10(title, body, json_key, keyword) or 0)
    text = f"{clean_html_text(title)} {clean_html_text(body)[:2600]} {keyword}"
    if v10_is_individual_crime_case(text, json_key):
        return round(base - 35, 2)

    signals = v10_detect_signals(text)
    group = v10_main_entity_group(text, json_key)
    family = v10_story_family_from_signals(signals, text, json_key) if signals else ""
    for signal in signals:
        weight = V10_SIGNAL_WEIGHTS.get(signal, 0)
        if group == "self":
            base += weight * 1.15
    ...
    # 단독/종합 같은 표현은 중요도 가산이지만, 특정 인물/오늘 이슈에는 의존하지 않는다.
    if re.search(r"\[단독\]|단독|종합|속보", title) and group in {"self", "government", "competitor"}:
        base += 6
    if group == "self" and family in {"labor_law", "legal_regulatory", "privacy_security", "leadership_service"}:
        base += 8
    if group == "self" and family == "financial_performance" and not re.search(r"실적|적자|흑자|영업손실|영업이익|목표가|공시", text):
        base -= 8
    if re.search(r"네이버\s*블로그|Naver\s*Blog|블로그", title, re.IGNORECASE):
        base -= 30
    if re.search(r"사진|포토|화보|캡션|영상만|현장사진", title, re.IGNORECASE):
        base -= 22
    return round(base, 2)
...
def is_article_obviously_low_value(article):
    ...
    if v10_is_individual_crime_case(text, article.get("JSON카테고리", "")):
        return True, "individual_crime_case_not_policy"
    must, _reason = v10_must_review_reason(text, article.get("JSON카테고리", ""))
    if must:
        return False, ""
    return _BASE_is_article_obviously_low_value_v10(article)
...
def is_report_item_relevant(report_item, json_key):
    ...
    if must:
        return True, ""
    return _BASE_is_report_item_relevant_v10(report_item, json_key)
...
def final_duplicate_reason(new_item, existing_items):
    # 먼저 기존 canonical/title/body 중복 기준을 적용한다.
    base_dup, base_reason = _BASE_final_duplicate_reason_v10(new_item, existing_items)
    if base_dup:
        return base_dup, base_reason

    new_bucket = v10_story_topic_bucket(new_item)
    if not new_bucket:
        return None, ""
    new_title = clean_html_text(new_item.get("기사제목", ""))
    new_anchors = v10_story_anchor_tokens(new_item)

    for old in existing_items:
        old_bucket = v10_story_topic_bucket(old)
        if old_bucket != new_bucket:
            continue
        old_title = clean_html_text(old.get("기사제목", ""))
        ratio = sequence_ratio(new_title, old_title)
        anchor_overlap = jaccard(new_anchors, v10_story_anchor_tokens(old))
        # 같은 범용 버킷이어도 제목/핵심 앵커가 겹칠 때만 같은 기사군으로 본다.
        if ratio >= 0.58 or anchor_overlap >= 0.38:
            return old, f"same_story_topic:{new_bucket}:ratio={round(ratio,2)}:anchor={round(anchor_overlap,2)}"
    return None, ""


def event_tag_limit_reason(new_item, existing_items):
    bucket = v10_story_topic_bucket(new_item)
    json_key = new_item.get("카테고리") or new_item.get("JSON카테고리") or ""

    if bucket:
        limit = V10_STORY_TOPIC_LIMITS.get(bucket)
        if limit is not None:
            count = sum(1 for old in existing_items if v10_story_topic_bucket(old) == bucket)
            if count >= limit:
                return f"event_story_topic_limit:{bucket}:{limit}"
        # 카테고리 내 유사 family 독식 방지. 특정 사건명이 아니라 family 단위로 제한한다.
        if ":" in bucket:
            _, family = bucket.split(":", 1)
            for (cat, prefix), total_limit in V10_CATEGORY_FAMILY_TOTAL_LIMITS.items():
                if cat == json_key and family.startswith(prefix):
                    total = 0
                    for old in existing_items:
                        old_bucket = v10_story_topic_bucket(old)
                        if old.get("카테고리") == json_key and old_bucket.endswith(old_bucket.split(":", 1)[-1]) and old_bucket.split(":", 1)[-1].startswith(prefix):
                            total += 1
                    if total >= total_limit:
                        return f"event_family_limit:{json_key}:{prefix}:{total_limit}"
    return _BASE_event_tag_limit_reason_v10(new_item, existing_items)
...
def v10_must_priority(article):
    ...
    score = 0.0
    for signal in signals:
        score += V10_SIGNAL_WEIGHTS.get(signal, 0)
    if group == "self":
        score *= 1.2
    elif group == "government":
        score *= 1.0
        ...
        score *= 0.6
    score += float(article.get("중요도점수") or 0) * 0.8
    score += float(article.get("랭킹원점수") or article.get("랭킹점수") or 0) * 0.25
    return score


def rank_and_trim_candidates(raw_articles):
    """v10: 일반 랭킹 + 범용 위험 신호 후보 + 카테고리 균형 후보를 혼합한다."""
    for article in raw_articles:
        ...
        article["스토리버킷"] = v10_story_topic_bucket(article)
        must, reason = v10_must_review_reason(text, article.get("JSON카테고리", ""))
        article["강제검토"] = "Y" if must else ""
        article["강제검토사유"] = reason
        if must:
            # must-review는 후보군에 들어오도록 보정하되, 특정 신호가 후보군을 독식하지 못하게 뒤에서 버킷 제한한다.
            article["랭킹점수"] = round(float(article.get("랭킹점수") or 0) + 28 + min(30, v10_must_priority(article) * 0.15), 2)
    ...
    def add(article, respect_topic_cap=True):
        ...
        if respect_topic_cap and topic:
            cap = V10_MUST_REVIEW_TOPIC_POOL_LIMIT if article.get("강제검토") == "Y" else max(8, V10_MUST_REVIEW_TOPIC_POOL_LIMIT // 2)
    ...
    # 1) 범용 must-review 후보: 특정 단어가 아니라 위험 신호/엔티티 기반으로 포함.
    for json_key in JSON_KEYS_ORDER:
        limit = MUST_REVIEW_POOL_LIMIT.get(json_key, 20)
        bucket = [
            a for a in ranked_all
            if a.get("JSON카테고리") == json_key
            and a.get("강제검토") == "Y"
            and float(a.get("랭킹점수") or 0) > -35
        ]
        bucket = sorted(bucket, key=lambda x: (v10_must_priority(x), float(x.get("랭킹점수") or 0)), reverse=True)
        added = 0
        for article in bucket:
            if added >= limit:
                break
    ...
    # 그래도 부족하면 topic cap을 한 번 완화하여 채움.
    if len(selected) < MAX_CANDIDATES_FOR_GEMINI:
        for article in ranked_all:
            if len(selected) >= MAX_CANDIDATES_FOR_GEMINI:
                break
    ...
    selected = selected[:MAX_CANDIDATES_FOR_GEMINI]
...
def enforce_selection_limits(selection, ranked_candidates):
    ...
    # Gemini가 놓친 must-review도 넣되, 범용 스토리버킷별로 과도한 독식을 막는다.
    for key in JSON_KEYS_ORDER:
        max_count = CATEGORY_MAX.get(key, CATEGORY_TARGET.get(key, 3))
        bucket_counts = {}
        ...
        must_bucket = [a for a in ranked_candidates if a.get("JSON카테고리") == key and a.get("강제검토") == "Y"]
        must_bucket = sorted(must_bucket, key=lambda x: (v10_must_priority(x), float(x.get("랭킹점수") or 0)), reverse=True)
        for article in must_bucket:
            ...
            topic_cap = 3 if key == "자사_및_계열사_이슈" else 2
            if topic and bucket_counts.get(topic, 0) >= topic_cap:
                continue
            result[key].append(art_id)
            used.add(art_id)
            if topic:
...
def main():
    ...
    print("   └ v10: 특정 사건명 하드코딩이 아니라 임원/조직, 서비스개편, 노무·법적진정, 규제·제재, 장애·보안, M&A/지배구조 등 범용 위험 신호 후보를 강제 검토합니다.")
...
[선별 관점: 카카오 대외협력/대관팀의 자아]
1. 카카오 및 계열사 이슈는 최우선으로 봐. 특히 경영진·임원 이동, 조직개편, 서비스 개편 논란, 노사/임단협/파업, 노동부 진정, 최저임금법, 고용노동부 근로감독, 소송·수사·과징금·개인정보·장애·피싱·지배구조·실적·매각·투자회수는 높게 평가해.
2. 강제검토=Y 후보는 특정 인물/오늘 사건명이 아니라 범용 위험 신호(임원·조직 변화, 서비스 개편 후폭풍, 노무·법적 진정, 규제·제재, 장애·보안, M&A·지배구조, 개인정보·AI·디지털자산 정책)에 걸린 기사야. 제목이 자극적이지 않아도 대관·대외협력 관점에서 반드시 검토해.
3. 정부/국회는 AI, 플랫폼, 디지털자산, 스테이블코인, 개인정보, 보안, 공정위, 금융위, 금감원, 과기정통부, 방미통위, 온플법, 망사용료, 지도반출 등 카카오 사업환경에 영향을 주는 정책·규제 중심으로 골라.
4. 경쟁사/해외이슈는 네이버·구글·오픈AI·MS·메타·애플·쿠팡·토스·배민·통신3사 등 주요 플레이어의 전략, 규제, 소송, 장애, 보안, AI/플랫폼 변화 위주로 골라.
5. 산업동향은 정말 구조적 변화가 있는 1~2개만 골라. 단순 인터뷰, 행사, 일반 제품 루머, 개별 범죄사건은 제외해.
