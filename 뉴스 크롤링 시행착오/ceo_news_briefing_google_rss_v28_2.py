...
# =========================================================
# v28 overrides: wider representative replacement + stricter recurring enforcement dedupe
# - When a selected issue's representative article fails body/alignment checks, search the full labeled article pool for
#   same event-key / issue-anchor replacements, not only articles already grouped into the same issue.
# - Publish bodyless critical items only after global replacement fails and only when they pass importance/weak-filler gates.
# - Platform/privacy/regulatory enforcement items from the 7-day index dedupe hard by base event key unless the stage
#   clearly advances to a new official outcome.
# - Tighten refill/final guardrails for weak government fillers and non-competitor MOU/PoC articles.
# =========================================================

V6_VERSION = "google_rss_v28_1_global_replacement_conditional_bodyless"

V28_ENFORCEMENT_STAGE_ORDER = {
    "general": 0,
    "investigation_review": 1,
    "consent_decree": 2,
    "sanction_penalty": 3,
    "litigation": 4,
}

V28_ANCHOR_STOPWORDS = {
    "정부", "국회", "국내", "한국", "오늘", "이번", "관련", "추진", "확대", "강화", "발표", "개최", "선정", "참여",
    "사업", "기술", "기업", "시장", "산업", "서비스", "플랫폼", "인공지능", "디지털", "정책", "규제", "협력", "논의",
    "기반", "구축", "진행", "계획", "전략", "분야", "이슈", "뉴스", "AI", "ICT", "데이터", "센터",
}

V28_CORE_ENTITY_RE = re.compile(
    r"카카오|카카오톡|카카오페이|카카오뱅크|카카오모빌리티|카카오게임즈|카카오엔터|두나무|업비트|"
    r"네이버|라인|쿠팡|쿠팡이츠|배달의민족|우아한형제들|토스|비바리퍼블리카|SKT|SK텔레콤|KT|LGU\+|"
    r"삼성전자|구글|Google|오픈AI|OpenAI|MS|마이크로소프트|Microsoft|메타|Meta|애플|Apple|앤트로픽|Anthropic|엔비디아|NVIDIA|"
    r"개보위|개인정보보호위원회|공정위|공정거래위원회|금융위|금융위원회|금감원|금융감독원|과기정통부|방미통위|방통위|중기부|국회|민주당|국민의힘|EU|미국|호주|독일|중국|일본",
    re.IGNORECASE,
)

V28_LOW_VALUE_GOV_SUPPORT_RE = re.compile(
    r"(지원사업|육성사업|미래\s*유니콘|국책\s*연구과제|과제\s*참여|최종\s*선정|사업\s*선정).{0,80}"
    r"(추진|확장|공략|성과|파트너|상장|솔루션|서비스|제품)",
    re.IGNORECASE,
)
V28_EVENT_EXPO_RE = re.compile(r"전시회|박람회|컨퍼런스|행사|개막|부스|체험존|채용박람회|포럼|세미나|학회", re.IGNORECASE)
V28_COMPETITOR_ENTITY_RE = re.compile(
    r"네이버|라인|쿠팡|쿠팡이츠|배달의민족|우아한형제들|토스|비바리퍼블리카|SKT|SK텔레콤|KT|LGU\+|삼성전자|"
    r"구글|Google|오픈AI|OpenAI|MS|마이크로소프트|Microsoft|메타|Meta|애플|Apple|앤트로픽|Anthropic|엔비디아|NVIDIA|X\b|트위터|틱톡|아마존|AWS|오라클|Oracle",
    re.IGNORECASE,
)
V28_GENERAL_MOU_POC_RE = re.compile(r"업무협약|MOU|기술실증|PoC|실증|협약|공동\s*검증|솔루션\s*적용|스마트글라스|물류공정|물류\s*자동화", re.IGNORECASE)
V28_AI_INFRA_STRONG_RE = re.compile(
    r"데이터센터|AI\s*데이터센터|AIDC|GPU|엔비디아|NVIDIA|전력|원전|10GW|GW급|슈퍼컴퓨팅|AI\s*팩토리|인프라|지급보증|리스|클라우드\s*용량|모델\s*개발에서|확보전",
    re.IGNORECASE,
)
...
def v28_text(obj):
    try:
        ...
        return v26_2_text(obj) if 'v26_2_text' in globals() else clean_html_text(str(obj or ''))


def v28_title(obj):
    if isinstance(obj, dict):
        return clean_html_text(obj.get('기사제목') or obj.get('title') or obj.get('top_title') or '')
    return ''


def v28_enforcement_stage(item):
    try:
        return v27_detect_enforcement_stage(v28_text(item))
    except Exception:
        return 'general'


def v28_enforcement_stage_order(stage):
    return V28_ENFORCEMENT_STAGE_ORDER.get(str(stage or 'general'), 0)


def v28_is_new_enforcement_stage(candidate, past):
    """Allow a repeated enforcement event only when it reached a materially new official stage."""
    cand_stage = v28_enforcement_stage(candidate)
    past_stage = v28_enforcement_stage(past)
    cand_order = v28_enforcement_stage_order(cand_stage)
    past_order = v28_enforcement_stage_order(past_stage)
    # Investigation/review previews repeated across days are not meaningful updates. Sanction result or litigation is.
    if cand_order > past_order and cand_stage in {'sanction_penalty', 'consent_decree', 'litigation'}:
        return True, f"v28_same_enforcement_new_stage:{past_stage}->{cand_stage}"
    return False, f"v28_same_enforcement_base_duplicate:{past_stage}->{cand_stage}"


_BASE_v26_index_duplicate_check_v28 = v26_index_duplicate_check

def v26_index_duplicate_check(candidate, recent_past_items, mode="rss"):
    # Harder policy for platform/privacy/regulatory enforcement recurrence.
    try:
        cand_base = v27_platform_enforcement_key(candidate, include_stage=False)
    except Exception:
        cand_base = ''
    if cand_base and recent_past_items:
        for past in recent_past_items:
            try:
                past_base = v27_platform_enforcement_key(past, include_stage=False)
            except Exception:
                past_base = ''
            if past_base and past_base == cand_base:
                is_update, reason = v28_is_new_enforcement_stage(candidate, past)
                if is_update:
                    return False, past, {"is_duplicate": False, "reason": reason, "score": 0.0, "shared_entities": "", "shared_tags": cand_base}
                return True, past, {"is_duplicate": True, "reason": reason, "score": 0.96, "shared_entities": "", "shared_tags": cand_base}
    return _BASE_v26_index_duplicate_check_v28(candidate, recent_past_items, mode=mode)


def v28_anchor_terms(issue, decision=None):
    parts = []
    for obj in [issue or {}, decision or {}]:
        if isinstance(obj, dict):
            parts.extend([
                obj.get('issue_group', ''), obj.get('issue_family', ''), obj.get('top_title', ''), obj.get('reason', ''),
                obj.get('category', ''), obj.get('v27_event_key', ''), obj.get('v26이벤트키', ''),
            ])
    text = clean_html_text(' '.join(str(x or '') for x in parts))
    terms = set()
    for m in V28_CORE_ENTITY_RE.finditer(text):
        terms.add(clean_html_text(m.group(0)))
    for tok in tokenize_for_similarity(text):
        tok = clean_html_text(tok)
        if len(tok) < 2 or tok in V28_ANCHOR_STOPWORDS:
            continue
...
def v28_article_event_key(article):
    for key in ['v27_event_key', 'v26이벤트키', 'event_key']:
        val = clean_html_text(article.get(key, '')) if isinstance(article, dict) else ''
        if val:
            return val
    try:
        return v26_event_key_for_item(article)
    except Exception:
        return ''


def v28_global_candidate_score(article, issue, decision, requested_json_key, anchor_terms, known_event_keys):
    text = v28_text(article)
    title = v28_title(article)
    if not text and not title:
        return -9999
    if v23_is_never_title_only(article):
        return -9999
    score = 0
    art_event = v28_article_event_key(article)
    if art_event and art_event in known_event_keys:
        score += 120
    try:
        base_key = v27_platform_enforcement_key(article, include_stage=False)
        if base_key and base_key in known_event_keys:
            score += 150
    except Exception:
        pass
    art_terms = set(tokenize_for_similarity(f"{title} {text[:900]}"))
    core_hits = set(m.group(0) for m in V28_CORE_ENTITY_RE.finditer(f"{title} {text[:900]}"))
    overlap = len((anchor_terms & art_terms) | (anchor_terms & core_hits))
    score += min(90, overlap * 12)
    if requested_json_key == JSON_KEYS_ORDER[0] and v26_has_strict_self_entity(text):
        score += 40
    if requested_json_key == JSON_KEYS_ORDER[1]:
        if v26_2_article_profile(article).get('gov_ok'):
            score += 45
        if v28_is_weak_government_item(article):
            score -= 120
    if requested_json_key == JSON_KEYS_ORDER[2]:
        if V28_COMPETITOR_ENTITY_RE.search(text):
            score += 45
        if v28_is_weak_competitor_item(article):
            score -= 120
    if requested_json_key == JSON_KEYS_ORDER[3]:
        try:
            ok, _reason, ind_score = v27_industry_basis(text, title)
            if ok:
                score += 35 + min(30, float(ind_score) * 3)
        except Exception:
            pass
        if V28_AI_INFRA_STRONG_RE.search(text):
            score += 50
    # Prefer fuller, extractable-looking sources, not title-only/portal noise.
    try:
        score += min(20, int(article.get('본문글자수') or article.get('body_len') or 0) / 300)
    except Exception:
        pass
    try:
        score += min(20, float(article.get('랭킹점수') or article.get('대표선택점수') or 0) / 20)
    except Exception:
        pass
    return score


def v28_global_replacement_candidate_ids(issue, decision, article_by_id, requested_json_key, existing_ids):
    anchor_terms = v28_anchor_terms(issue, decision)
    known_event_keys = set()
    for obj in [issue or {}, decision or {}]:
        if isinstance(obj, dict):
            for key in ['v27_event_key', 'v26이벤트키', 'event_key', 'issue_group', 'issue_family']:
                val = clean_html_text(obj.get(key, ''))
                if val:
                    known_event_keys.add(val)
    for art_id in existing_ids:
        art = article_by_id.get(int(art_id))
        if art:
            ev = v28_article_event_key(art)
            if ev:
                known_event_keys.add(ev)
            try:
                base = v27_platform_enforcement_key(art, include_stage=False)
                if base:
                    known_event_keys.add(base)
            except Exception:
                pass
    scored = []
    existing = {int(x) for x in existing_ids if str(x).isdigit()}
    for art_id, art in article_by_id.items():
        try:
            aid = int(art_id)
        except Exception:
            continue
        if aid in existing:
            continue
        s = v28_global_candidate_score(art, issue, decision, requested_json_key, anchor_terms, known_event_keys)
        if s >= 45:
            scored.append((s, aid))
    scored.sort(reverse=True)
    return [aid for _s, aid in scored[:35]]


def v28_is_weak_government_item(item):
    t = v28_text(item)
    title = v28_title(item)
    p = v26_2_article_profile(item)
    # Single-company support-program / R&D-selected PR is weak for the CEO briefing unless it is a market-level public project.
    if V28_LOW_VALUE_GOV_SUPPORT_RE.search(f"{title} {t[:1000]}"):
        if not re.search(r"카카오|네이버|SKT|KT|쿠팡|토스|배달의민족|플랫폼\s*사업자|AI\s*컴퓨팅센터|국가AI|데이터센터|규제|의무|법안|과징금|제재", t, re.I):
            return True
    # Event/expo notes are weak unless the policy decision itself is the news.
    if V28_EVENT_EXPO_RE.search(f"{title} {t[:900]}") and not re.search(r"법안|규제|제재|과징금|시행|가이드라인|전략|예산\s*투입|국가AI|AI\s*컴퓨팅센터", t, re.I):
        return True
    # General banking/financial probe not linked to digital/platform/AI/privacy is too far from this briefing.
    if re.search(r"우리금융|우리은행|은행권|사회공헌|광고비|브랜드\s*광고", t, re.I) and not v27_is_digital_platform_context(t):
        return True
    if not p.get('gov_ok'):
        return True
    return False
...
def v28_is_weak_competitor_item(item):
    t = v28_text(item)
    if V28_COMPETITOR_ENTITY_RE.search(t):
        return False
    # Public-sector overseas cloud/AI MOU can remain if structural and overseas.
    if v26_2_is_overseas_structural_text(t, v28_title(item)):
        return False
    if V28_GENERAL_MOU_POC_RE.search(t):
        return True
    return False


def v28_is_bodyless_item(item):
    method = str(item.get('본문추출방식') or item.get('body_status') or '')
    body_len = v20_int(item.get('본문글자수'), 0)
    return method == 'critical_without_body' or body_len <= 0 or str(item.get('본문상태','')).startswith('본문추출실패')


# v28.1: Bodyless is allowed only for truly important issues after all global replacements fail.
# This keeps critical CEO-relevant items (for example Kakao direct incidents, major regulation, or big-tech AI moves)
# while preventing weak fillers, PR, event notes, generic MOU/PoC, and stock-market items from entering title-only.
def v28_1_bodyless_importance_reason(issue, decision, article_info, requested_json_key):
    text = clean_html_text(" ".join(str(x or "") for x in [
        (article_info or {}).get('기사제목', ''),
        (article_info or {}).get('본문요약', ''),
        (article_info or {}).get('검색어', ''),
        (issue or {}).get('issue_group', ''),
        (issue or {}).get('issue_family', ''),
        (issue or {}).get('summary', ''),
        (decision or {}).get('reason', ''),
    ]))
    title = clean_html_text((article_info or {}).get('기사제목', '') or (issue or {}).get('top_title', ''))
    tmp = {
        '카테고리': requested_json_key,
        '기사제목': title,
        '본문요약': text,
        ...
        '본문글자수': 0,
        '본문추출방식': 'critical_without_body',
        '링크': (article_info or {}).get('링크', ''),
        '검색어': (article_info or {}).get('검색어', ''),
    }

    if not article_info:
        return False, 'no_fallback_article_for_bodyless'
    try:
        if v23_is_never_title_only(article_info):
            return False, 'never_allow_title_only_low_value_or_caption'
    except Exception:
        pass
    try:
        if v14_is_pure_market_or_stock_noise(text, url=(article_info or {}).get('링크',''), json_key=requested_json_key, keyword=(article_info or {}).get('검색어','')):
            return False, 'bodyless_stock_market_noise'
    except Exception:
        if re.search(r'특징주|목표가|투자의견|주가\s*(급등|급락|상승|하락|전망)|증시|장중|매수\s*의견|매도\s*의견', text, re.I):
            return False, 'bodyless_stock_market_noise_regex'

    # Section-specific weak-filler guards remain strict for title/RSS-only entries.
    if requested_json_key == JSON_KEYS_ORDER[1] and v28_is_weak_government_item(tmp):
        return False, 'bodyless_weak_government_filler'
    if requested_json_key == JSON_KEYS_ORDER[2] and v28_is_weak_competitor_item(tmp):
        return False, 'bodyless_weak_competitor_filler'
    if requested_json_key == JSON_KEYS_ORDER[3]:
        # Industry bodyless needs stronger structural signal than ordinary body-success items.
        if not (V28_AI_INFRA_STRONG_RE.search(text) or v26_2_is_overseas_structural_text(text, title)):
            return False, 'bodyless_weak_industry_without_structural_signal'

    priority = v20_float((decision or {}).get('priority'), 0)
    issue_score = v20_float((issue or {}).get('issue_score'), 0)
    critical_by_model = priority >= 4.5 or issue_score >= 70 or v20_is_critical_issue(issue or {}, decision or {})

    # Concrete high-importance signals. These are categories of events, not one-day title patches.
    self_direct = requested_json_key == JSON_KEYS_ORDER[0] and re.search(
        r'카카오|카카오톡|카카오페이|카카오뱅크|카카오모빌리티|카카오게임즈|카카오엔터|두나무|업비트', text, re.I
    ) and re.search(
        r'파업|노조|임단협|장애|오류|먹통|개인정보|보안|수사|소송|제재|과징금|대표|임원|조직|매각|지분|인수|합병|협력|오픈AI|OpenAI|샘\s*올트먼', text, re.I
    )
    gov_core = requested_json_key == JSON_KEYS_ORDER[1] and re.search(
        r'개보위|개인정보보호위원회|공정위|공정거래위원회|금융위|금융위원회|금감원|금융감독원|과기정통부|방미통위|방통위|국회|민주당|국민의힘|정부|부총리|장관|위원회', text, re.I
    ) and re.search(
        r'법안|입법|시행|의결|통과|제재|과징금|조사|심의|점검|감독|가이드라인|정책협의체|기본법|온플법|개인정보|AI|디지털자산|스테이블코인|플랫폼', text, re.I
    )
    comp_core = requested_json_key == JSON_KEYS_ORDER[2] and V28_COMPETITOR_ENTITY_RE.search(text) and re.search(
        r'AI|인공지능|데이터센터|GPU|엔비디아|투자|협력|규제|등급|보안|클라우드|플랫폼|검색|광고|커머스|앱스토어|챗GPT|모델|인프라', text, re.I
    )
    industry_core = requested_json_key == JSON_KEYS_ORDER[3] and (
        V28_AI_INFRA_STRONG_RE.search(text) or re.search(r'AI\s*인프라|데이터센터|GPU|전력|클라우드|보안|공급망|플랫폼\s*생태계', text, re.I)
    )

    if self_direct:
        return True, 'bodyless_allowed_self_direct_critical'
    if gov_core and critical_by_model:
        return True, 'bodyless_allowed_government_core_critical'
    if comp_core and critical_by_model:
        return True, 'bodyless_allowed_competitor_overseas_core_critical'
    if industry_core and critical_by_model:
        return True, 'bodyless_allowed_industry_structural_critical'

    return False, f'bodyless_not_critical_enough:priority={priority},issue_score={issue_score}'


def v28_1_is_allowed_bodyless_final_item(item):
    return str((item or {}).get('v28_1_bodyless_policy', '')).startswith('allowed_critical')


_BASE_v26_1_is_good_refill_candidate_v28 = v26_1_is_good_refill_candidate

def v26_1_is_good_refill_candidate(decision, issue, cat=None):
    if not _BASE_v26_1_is_good_refill_candidate_v28(decision, issue, cat=cat):
        return False
    cat = cat or v26_1_decision_category(decision, issue)
    text = clean_html_text(f"{issue.get('issue_group','')} {issue.get('top_title','')} {issue.get('issue_family','')} {issue.get('summary','')}")
    if cat == JSON_KEYS_ORDER[1]:
        if v28_is_weak_government_item({'기사제목': issue.get('top_title',''), '본문전문': text, '본문요약': text}):
            return False
    if cat == JSON_KEYS_ORDER[2]:
        if v28_is_weak_competitor_item({'기사제목': issue.get('top_title',''), '본문전문': text, '본문요약': text}):
            return False
    return True


# Replace v20_process_issue with a v28.1 version that expands candidate search globally and allows critical bodyless only conditionally.
def v20_process_issue(decision, issue_by_id, article_by_id, recent_past_items, final_report_data, body_failed_rows, skip_rows, processed_article_ids, order_counter):
    issue = issue_by_id.get(decision.get("issue_id"))
    if not issue:
        return None, order_counter, "missing_issue"
    ...
    def add_id(x):
        try:
            ...
            return
        if ix not in candidate_ids:
            candidate_ids.append(ix)
    add_id(decision.get("best_article_id"))
    for source_ids in [decision.get("backup_article_ids", []), issue.get("candidate_article_ids", []), issue.get("article_ids", [])]:
        for art_id in source_ids or []:
            add_id(art_id)
    # v28: search full article pool for same-event/anchor replacements before giving up.
    for art_id in v28_global_replacement_candidate_ids(issue, decision, article_by_id, requested_json_key, candidate_ids):
        add_id(art_id)
    ...
    for art_id in candidate_ids:
        ...
        if v23_is_never_title_only(article_info):
            ...
            v23_add_diagnostic_columns(failed, article_info, requested_json_key, "fail:never_allow_title_only")
            failed["제외단계"] = "v28_representative_attempt"
            failed["v20_issue_id"] = issue.get("issue_id")
            failed["v20_issue_group"] = issue.get("issue_group")
            body_failed_rows.append(failed)
            attempted_failed.append((article_info, "never_allow_title_only"))
        ...
        report_item, skip_info, failed_item = process_article_for_report(article_info, requested_json_key, recent_past_items)
        if skip_info:
            skip_info["제외단계"] = "v28_representative_attempt"
            skip_info["v20_issue_id"] = issue.get("issue_id")
            skip_info["v20_issue_group"] = issue.get("issue_group")
            skip_rows.append(skip_info)
            attempted_failed.append((article_info, skip_info.get('제외사유', 'past_duplicate_or_skip')))
            continue
        if failed_item:
            failed_item["제외단계"] = "v28_representative_attempt"
            failed_item["v20_issue_id"] = issue.get("issue_id")
            failed_item["v20_issue_group"] = issue.get("issue_group")
            v23_add_diagnostic_columns(failed_item, article_info, requested_json_key, failed_item.get("최종카테고리검증결과", ""))
            body_failed_rows.append(failed_item)
        ...
        report_item["선정순서"] = order_counter
        report_item["본문상태"] = "본문추출성공"
        report_item["v28대표대체후보수"] = len(candidate_ids)

        action, guarded_category, guard_reason = v23_final_category_guardrail(report_item, requested_json_key)
        if action == "fail":
            failed_copy = dict(report_item)
            failed_copy["본문품질사유"] = f"category_guardrail_failed:{guard_reason}"
            failed_copy["제외단계"] = "v28_representative_attempt"
            failed_copy["최종카테고리검증결과"] = f"fail:{guard_reason}"
            body_failed_rows.append(failed_copy)
            attempted_failed.append((article_info, guard_reason))
            continue
        ...
        # v28 extra class-level hard filters.
        final_cat = report_item.get('카테고리')
        if final_cat == JSON_KEYS_ORDER[1] and v28_is_weak_government_item(report_item):
            failed_copy = dict(report_item)
            failed_copy["본문품질사유"] = "v28_weak_government_filler_rejected"
            failed_copy["제외단계"] = "v28_representative_attempt"
            body_failed_rows.append(failed_copy)
            attempted_failed.append((article_info, "v28_weak_government_filler_rejected"))
            continue
        if final_cat == JSON_KEYS_ORDER[2] and v28_is_weak_competitor_item(report_item):
            failed_copy = dict(report_item)
            failed_copy["본문품질사유"] = "v28_weak_competitor_mou_poc_rejected"
            failed_copy["제외단계"] = "v28_representative_attempt"
            body_failed_rows.append(failed_copy)
            attempted_failed.append((article_info, "v28_weak_competitor_mou_poc_rejected"))
            continue
        ...
        if align_action == "fail":
            failed_copy = dict(report_item)
            failed_copy["본문품질사유"] = f"representative_alignment_failed:{align_reason}"
            failed_copy["제외단계"] = "v28_representative_alignment"
            body_failed_rows.append(failed_copy)
            attempted_failed.append((article_info, f"representative_alignment_failed:{align_reason}"))
            continue
        if align_action == "weak" and len(candidate_ids) > 1:
        ...
        dup_item, dup_reason = v21_strict_cross_issue_duplicate(report_item, final_report_data)
        if dup_item:
            new_score = v20_float(report_item.get("대표선택점수"), 0) + v27_category_precedence_bonus(report_item)
            old_score = v20_float(dup_item.get("대표선택점수"), 0) + v27_category_precedence_bonus(dup_item)
            if dup_reason == "same_gemini_issue_group" and new_score > old_score + REPRESENTATIVE_REPLACE_MARGIN:
                try:
                    idx = final_report_data.index(dup_item)
                    final_report_data[idx] = report_item
                    ...
                    final_report_data.append(report_item)
                add_duplicate_skip_row(skip_rows, dup_item, report_item, f"replaced_by_better_duplicate:{dup_reason}", stage="v28_issue_selected", extra=f"old={old_score},new={new_score}")
                return report_item, order_counter + 1, "replaced_duplicate"
            add_duplicate_skip_row(skip_rows, report_item, dup_item, f"strict_duplicate:{dup_reason}", stage="v28_issue_selected", extra=f"old={old_score},new={new_score}")
            return None, order_counter, "duplicate_already_represented"

        final_report_data.append(report_item)
        return report_item, order_counter + 1, "body_success"

    # v28.1: after global replacement fails, allow bodyless only for genuinely critical issues.
    fallback_id = None
    try:
        fallback_id = int(decision.get("best_article_id") or issue.get("top_article_id"))
    except Exception:
        fallback_id = None
    article_info = article_by_id.get(fallback_id) if fallback_id is not None else None

    bodyless_ok, bodyless_reason = v28_1_bodyless_importance_reason(issue, decision, article_info, requested_json_key)
    if bodyless_ok and article_info:
        item = v20_create_bodyless_report_item(
            article_info,
            issue,
            decision,
            requested_json_key,
            reason=f"v28_1_after_global_replacement_failed:{bodyless_reason}",
        )
        item["v28_1_bodyless_policy"] = f"allowed_critical:{bodyless_reason}"
        item["v28대표대체후보수"] = len(candidate_ids)
        item["본문상태"] = "본문추출실패_중요이슈_RSS제한포함"
        item["본문품질사유"] = f"global_replacement_failed_{len(candidate_ids)}_candidates; {bodyless_reason}"

        action, guarded_category, guard_reason = v23_final_category_guardrail(item, requested_json_key)
        if action == "fail":
            bodyless_ok = False
            bodyless_reason = f"bodyless_guardrail_failed:{guard_reason}"
        else:
            if action == "reassign" and guarded_category:
                v23_set_final_category(item, guarded_category, guard_reason)
                item["최종카테고리검증결과"] = f"bodyless_reassign:{guard_reason}"
            ...
            # Re-check weak-filler policy after category guardrail/reassign.
            final_cat = item.get('카테고리')
            if final_cat == JSON_KEYS_ORDER[1] and v28_is_weak_government_item(item):
                bodyless_ok = False
                bodyless_reason = "bodyless_weak_government_after_reassign"
            elif final_cat == JSON_KEYS_ORDER[2] and v28_is_weak_competitor_item(item):
                bodyless_ok = False
                bodyless_reason = "bodyless_weak_competitor_after_reassign"
            elif final_cat == JSON_KEYS_ORDER[3] and not (V28_AI_INFRA_STRONG_RE.search(v28_text(item)) or v26_2_is_overseas_structural_text(v28_text(item), v28_title(item))):
                bodyless_ok = False
                bodyless_reason = "bodyless_weak_industry_after_reassign"
        ...
        if bodyless_ok:
            ...
            if align_action == "fail":
                bodyless_ok = False
                bodyless_reason = f"bodyless_alignment_failed:{align_reason}"

        if bodyless_ok:
            # Important bodyless items still obey recent-index and cross-issue duplicate policy.
            past_dup, past_item, dup_diag = v26_index_duplicate_check(item, recent_past_items, mode="bodyless")
            if past_dup:
                add_duplicate_skip_row(skip_rows, item, past_item, dup_diag.get('reason', 'bodyless_index_duplicate'), stage="critical_without_body")
                return None, order_counter, "bodyless_index_duplicate"
            dup_item, dup_reason = v21_strict_cross_issue_duplicate(item, final_report_data)
            if dup_item:
                add_duplicate_skip_row(skip_rows, item, dup_item, f"bodyless_duplicate:{dup_reason}", stage="critical_without_body")
                return None, order_counter, "bodyless_duplicate"
    ...
    failed = {
        ...
        "카테고리명": JSON_KEY_TO_DISPLAY.get(requested_json_key, requested_json_key),
        "검색어": (article_info or {}).get("검색어", ""),
        "기사제목": (article_info or {}).get("기사제목", issue.get('top_title', '')),
        "언론사": (article_info or {}).get("언론사", ""),
        "게시일": (article_info or {}).get("게시일", ""),
        "RSS요약": (article_info or {}).get("본문요약", ""),
        "본문전문": "",
        "본문글자수": 0,
        "본문추출방식": "v28_1_bodyless_rejected",
        "본문품질점수": -100,
        "본문품질사유": f"all_replacement_candidates_failed:{len(candidate_ids)} tried; {bodyless_reason}",
        "본문사용가능": False,
        "본문상태": "최종제외_REVIEW_REQUIRED",
        "원래RSS링크": (article_info or {}).get("링크", ""),
        "링크": (article_info or {}).get("링크", ""),
        "제외단계": "v28_1_all_candidates_failed_bodyless_rejected",
        "v20_issue_id": issue.get("issue_id"),
        "v20_issue_group": issue.get("issue_group"),
        "v28시도후보수": len(candidate_ids),
    }
    if article_info:
        try:
            v23_add_diagnostic_columns(failed, article_info, requested_json_key, "v28_1_bodyless_rejected")
        except Exception:
            pass
    body_failed_rows.append(failed)
    return None, order_counter, "all_candidates_failed_bodyless_rejected"
...
def v21_remove_obvious_final_duplicates(final_items):
    ...
    cleaned = []
    for item in kept:
        if v28_is_bodyless_item(item) and not v28_1_is_allowed_bodyless_final_item(item):
            removed.append((item, 'v28_1_unapproved_bodyless_final_removed'))
            continue
        if item.get('카테고리') == JSON_KEYS_ORDER[1] and v28_is_weak_government_item(item):
            removed.append((item, 'v28_weak_government_filler_removed'))
            continue
        if item.get('카테고리') == JSON_KEYS_ORDER[2] and v28_is_weak_competitor_item(item):
            removed.append((item, 'v28_weak_competitor_mou_poc_removed'))
            continue
        if item.get('카테고리') == JSON_KEYS_ORDER[3] and v28_is_bodyless_item(item):
            txt = v28_text(item)
            if not (V28_AI_INFRA_STRONG_RE.search(txt) or v26_2_is_overseas_structural_text(txt, v28_title(item))):
                removed.append((item, 'v28_1_weak_industry_bodyless_removed'))
                continue
        cleaned.append(item)
    # Re-sort after removals with existing priority function.
    try:
        cleaned = sorted(cleaned, key=v26_final_item_priority_score, reverse=True)
    except Exception:
        pass
    return cleaned, removed


_BASE_v26_2_industry_pivot_score_v28 = v26_2_industry_pivot_score

def v26_2_industry_pivot_score(item):
    s = float(_BASE_v26_2_industry_pivot_score_v28(item) or 0)
    t = v28_text(item)
    if V28_AI_INFRA_STRONG_RE.search(t):
        s += 120
    if re.search(r"사내|임직원|업무\s*도입|업무\s*혁신", t, re.I) and not V28_AI_INFRA_STRONG_RE.search(t):
        s -= 35
    return s
...
def v20_gemini_edit_issues(client, issues, recent_past_text):
    # Keep the v27 prompt structure but append stronger operational lessons from the 2026-06-11 review.
    if not client:
        raise RuntimeError('Gemini client 없음')
    visible_issues = [i for i in sorted(issues, key=lambda x: x.get('issue_score', 0), reverse=True) if not i.get('exclude') and not i.get('is_pr')]
    visible_issues = visible_issues[:V20_MAX_ISSUES_FOR_EDITOR]
    issue_text = '\n'.join(v20_issue_line(i) for i in visible_issues)
    prompt = f"""
You are editing a daily Korean CEO/public-affairs briefing for Kakao. Select issue groups, not just articles. Return JSON only.

Operational lessons:
- Prefer clean direct representative articles. If the best article may fail body extraction, provide 2-5 backup_article_ids from the same issue whenever available.
- Do not select a bodyless/title-only article if similar articles exist. If no extractable representative exists, a truly critical issue may remain title/RSS-based, but weak fillers must be dropped.
- Same enforcement/privacy/platform-regulation event in the recent index should not repeat merely because the title wording changed. Keep it only when the official stage materially changed, e.g. sanction amount/decision announced or litigation filed.
- For Government/National Assembly, do not fill slots with single-company support-program PR, event/expo notes, or non-digital banking probes. Use policy/regulation/legislation/public project direction.
- Public-private MOU/project participation is allowed only when the public actor/project is central to the news and relevant to Kakao's digital/AI/platform/security/fintech environment.
- For Competitor/Overseas, avoid generic MOU/PoC articles without a major competitor/global platform actor. Prefer Naver/SKT/KT/Coupang/Toss/Baemin/Google/OpenAI/MS/Meta/Apple/Anthropic/Nvidia and major platform/regulatory moves.
- For Industry, prioritize structural AI/platform/cloud/data-center/security changes. AI data-center/GPU/power/infrastructure shifts outrank ordinary internal enterprise AI adoption.

Recommended size: 11-14 total; Self 2-4, Gov 3-4, Competitor/Overseas 3-4, Industry 1-2. Do not force weak fillers.

JSON format:
{{
  "selected_issues": [
    {{"issue_id": "I001", "category": "자사_및_계열사_이슈/정부_국회/경쟁사_해외이슈/산업동향", "priority": 5, "best_article_id": 123, "backup_article_ids": [124,125], "reason": "why"}}
  ],
  "backup_issues": [
    {{"issue_id": "I050", "category": "경쟁사_해외이슈", "priority": 4, "best_article_id": 555, "backup_article_ids": [556], "reason": "backup reason"}}
  ]
}}
...
# =========================================================
# v28.2 overrides: strict same-event replacement + post-replacement dedupe
# - v28.1 widened global replacement too much; broad anchors such as 카카오/AI/정책 caused unrelated representatives.
# - This patch keeps global replacement, but only when a strict event identity check passes.
# - It also recalculates a final_event_key from the representative article itself and dedupes across sections.
# =========================================================

V6_VERSION = "google_rss_v28_2_strict_same_event_replacement_dedupe"

V28_2_GENERIC_EVENT_WORDS = {
    "카카오", "네이버", "구글", "오픈AI", "MS", "애플", "메타", "정부", "국회", "AI", "인공지능",
    "플랫폼", "정책", "규제", "서비스", "사업", "기술", "협력", "추진", "계획", "시장", "산업",
    "데이터", "센터", "기업", "관련", "오늘", "내일", "이번", "발표", "진행", "강화", "확대",
}

V28_2_KAKAO_LABOR_RE = re.compile(r"카카오|카카오페이|카카오엔터프라이즈|디케이테크인|엑스엘게임즈", re.I)
V28_2_LABOR_ACTION_RE = re.compile(r"노조|파업|부분파업|총파업|쟁의|쟁의권|임단협|성과급|RSU|조정|노동위|지노위|행진|집회|고용안정|업무\s*로그아웃|로그오프", re.I)
V28_2_DUNAMU_RE = re.compile(r"두나무|업비트|네이버파이낸셜|주식교환|지분구조|지분\s*구조|카카오인베스트먼트", re.I)
V28_2_DUNAMU_ACTION_RE = re.compile(r"지분|매각|구주|주식교환|경영권|자회사|완전\s*자회사|대주주|기업결합|심사", re.I)
V28_2_OPENAI_KAKAO_RE = re.compile(r"샘\s*올트먼|올트먼|오픈AI|OpenAI|챗GPT|ChatGPT", re.I)
V28_2_POLICY_COUNCIL_RE = re.compile(r"과기정통부|과학기술정보통신부", re.I)
V28_2_BMTC_RE = re.compile(r"방미통위|방송미디어통신위원회|방통위", re.I)
V28_2_COUNCIL_ACTION_RE = re.compile(r"정책협의|협의체|정책협의회|착수회의|AI\s*데이터|데이터\s*공유|K-FAST|미디어\s*생태계|OTT", re.I)
V28_2_COUPANG_PRIVACY_RE = re.compile(r"쿠팡|쿠팡이츠", re.I)
V28_2_PRIVACY_ENFORCE_RE = re.compile(r"개인정보|타사\s*웹|앱\s*이용기록|과징금|개보위|개인정보보호위원회|제재|시정명령|무단수집|맞춤형\s*광고", re.I)
V28_2_ONPLAW_RE = re.compile(r"온플법|온라인\s*플랫폼법|플랫폼법|대형\s*플랫폼|수수료|광고비", re.I)
V28_2_DIGITAL_ASSET_RE = re.compile(r"디지털자산|가상자산|스테이블코인|특금법|FIU|금융정보분석원|디지털자산기본법|거래소\s*대주주|원화\s*스테이블", re.I)
V28_2_NAVER_NVIDIA_RE = re.compile(r"네이버|NAVER", re.I)
V28_2_NVIDIA_AI_FACTORY_RE = re.compile(r"엔비디아|NVIDIA|젠슨\s*황|AI\s*팩토리|GPU|GW급|데이터센터", re.I)
V28_2_SKT_ANTHROPIC_RE = re.compile(r"SKT|SK텔레콤", re.I)
V28_2_ANTHROPIC_RE = re.compile(r"앤트로픽|Anthropic|클로드|Claude", re.I)
V28_2_X_RATING_RE = re.compile(r"\bX\b|엑스|트위터|Twitter", re.I)
V28_2_AGE_RATING_RE = re.compile(r"청소년\s*이용불가|19세|16세|15세|이용\s*등급|등급\s*상향|구글\s*플레이|플레이스토어|앱마켓", re.I)
V28_2_OPENAI_INFRA_RE = re.compile(r"오픈AI|OpenAI|챗GPT|ChatGPT", re.I)
V28_2_AI_INFRA_RE = re.compile(r"데이터센터|AI\s*데이터센터|10GW|GW|엔비디아|NVIDIA|지급보증|전력|리스|연방정부\s*부지|AI\s*인프라", re.I)
V28_2_BROADCOM_INFRA_RE = re.compile(r"브로드컴|Broadcom|앤트로픽|Anthropic|AI\s*XPV|플루이드스택|Fluidstack|XPU|TPU|아폴로|블랙스톤", re.I)
V28_2_SAMSUNG_AI_RE = re.compile(r"삼성전자|챗GPT|제미나이|Gemini|클로드|Claude|생성형\s*AI|임직원", re.I)
V28_2_MS_AU_RE = re.compile(r"마이크로소프트|Microsoft|\bMS\b", re.I)
V28_2_AU_CYBER_RE = re.compile(r"호주|사이버보안|클라우드|데이터센터|MOU|양해각서|애저|Azure", re.I)
V28_2_OPENAI_GOV_SHARE_RE = re.compile(r"오픈AI|OpenAI|샘\s*올트먼|올트먼", re.I)
V28_2_GOV_SHARE_RE = re.compile(r"지분\s*50|공공\s*소유|국부펀드|AI\s*사회주의|정부와의\s*지분", re.I)
V28_2_LOGISTICS_POC_RE = re.compile(r"딥파인|LX판토스|스마트글라스|물류공정|물류\s*자동화|기술실증|PoC", re.I)
V28_2_PUBLIC_AI_GUIDE_RE = re.compile(r"행안부|행정안전부", re.I)
V28_2_PUBLIC_AI_GUIDE_ACTION_RE = re.compile(r"공공부문\s*AI|AI\s*도입.*가이드|활용\s*가이드|범정부\s*인공지능\s*공통\s*기반|RAG", re.I)


def v28_2_base_text(obj):
    return v28_text(obj) if 'v28_text' in globals() else v26_2_text(obj)


def v28_2_title(obj):
    return v28_title(obj) if 'v28_title' in globals() else clean_html_text((obj or {}).get('기사제목', ''))


def v28_2_custom_event_key_from_text(text, category=""):
    t = clean_html_text(text)
    if not t:
        return ""
    # Self/Kakao direct issues
    if V28_2_KAKAO_LABOR_RE.search(t) and V28_2_LABOR_ACTION_RE.search(t):
        return "self_labor:kakao:strike_compensation"
    if V28_2_DUNAMU_RE.search(t) and V28_2_DUNAMU_ACTION_RE.search(t):
        return "self_strategy:dunamu_stake_naver_exchange"
    if SELF_KAKAO_PATTERN.search(t) and V28_2_OPENAI_KAKAO_RE.search(t) and re.search(r"방한|만난다|협력|카카오톡|연계|정신아", t, re.I):
        return "self_ai:openai_kakao_meeting"

    # Government / policy issues
    if V28_2_POLICY_COUNCIL_RE.search(t) and V28_2_BMTC_RE.search(t) and V28_2_COUNCIL_ACTION_RE.search(t):
        return "gov_policy:msit_bmtc_ai_media_council"
    if V28_2_COUPANG_PRIVACY_RE.search(t) and V28_2_PRIVACY_ENFORCE_RE.search(t):
        return "gov_enforcement:coupang_privacy_adtech_penalty"
    if V28_2_DIGITAL_ASSET_RE.search(t) and re.search(r"법|TF|해체|표류|입법|당정|대주주|발행\s*주체|제도화|논의|규제", t, re.I):
        return "gov_policy:digital_asset_legislation"
    if V28_2_ONPLAW_RE.search(t) and re.search(r"제정|법제화|국회|정무위|소상공인|수수료|광고비", t, re.I):
        return "gov_policy:online_platform_act"
    if V28_2_PUBLIC_AI_GUIDE_RE.search(t) and V28_2_PUBLIC_AI_GUIDE_ACTION_RE.search(t):
        return "gov_policy:public_sector_ai_guide"

    # Competitor / overseas issues
    if V28_2_NAVER_NVIDIA_RE.search(t) and V28_2_NVIDIA_AI_FACTORY_RE.search(t):
        return "competitor:naver_nvidia_ai_factory"
    if V28_2_SKT_ANTHROPIC_RE.search(t) and V28_2_ANTHROPIC_RE.search(t):
        return "competitor:skt_anthropic_ai_investment"
    if V28_2_X_RATING_RE.search(t) and V28_2_AGE_RATING_RE.search(t):
        return "competitor:google_x_age_rating"
    if V28_2_MS_AU_RE.search(t) and V28_2_AU_CYBER_RE.search(t):
        return "overseas:ms_australia_ai_cyber_cloud_mou"
    if V28_2_OPENAI_GOV_SHARE_RE.search(t) and V28_2_GOV_SHARE_RE.search(t):
        return "overseas:openai_public_equity_debate"

    # Industry / structure
    if V28_2_OPENAI_INFRA_RE.search(t) and V28_2_AI_INFRA_RE.search(t):
        return "industry:openai_ai_datacenter_infra"
    if V28_2_BROADCOM_INFRA_RE.search(t) and re.search(r"AI\s*인프라|20GW|53조|350억\s*달러|데이터센터|연산\s*용량", t, re.I):
        return "industry:broadcom_anthropic_ai_infra"
    if V28_2_SAMSUNG_AI_RE.search(t) and re.search(r"공식\s*도입|업무\s*혁신|임직원", t, re.I):
        return "industry:samsung_enterprise_genai_adoption"
    if V28_2_LOGISTICS_POC_RE.search(t):
        return "weak:logistics_smartglass_poc"
    return ""


def v28_2_final_event_key(item):
    """Canonical event key based on the actual representative article, not the selected issue label."""
    if not item:
        return ""
    text = v28_2_base_text(item)
    custom = v28_2_custom_event_key_from_text(text, (item or {}).get('카테고리', '') if isinstance(item, dict) else '')
    if custom:
        return custom
    try:
        k = v27_platform_enforcement_key(item, include_stage=False)
        if k:
            return f"v27_platform:{k}"
    except Exception:
        pass
    try:
        k = v15_privacy_security_base_key(item)
        if k:
            return f"v15_privacy:{k}"
    except Exception:
        pass
    try:
        k = v16_global_incident_base_key(item)
        if k:
            return f"v16_global:{k}"
    except Exception:
        pass
    return ""


def v28_2_issue_event_keys(issue, decision, article_by_id=None):
    keys = set()
    intent_text = clean_html_text(" ".join(str(x or '') for x in [
        (issue or {}).get('issue_group'), (issue or {}).get('issue_family'), (issue or {}).get('top_title'),
        (issue or {}).get('label_reasons'), (issue or {}).get('internal_category'),
        (decision or {}).get('reason'), (decision or {}).get('category'),
    ]))
    custom = v28_2_custom_event_key_from_text(intent_text, (decision or {}).get('category', ''))
    if custom:
        keys.add(custom)
    # Only trust explicit event keys, not broad issue_group/issue_family labels.
    for obj in [issue or {}, decision or {}]:
        if not isinstance(obj, dict):
            continue
        for key in ['v27_event_key', 'v26이벤트키', 'event_key', '전역사건기본키', '개인정보보안사건키']:
            val = clean_html_text(obj.get(key, ''))
            if val and len(val) >= 12 and not val.startswith('global_incident:platform_regulation:toss'):
                keys.add(val)
    if article_by_id and decision:
        for raw_id in [(decision or {}).get('best_article_id')]:
            try:
                art = article_by_id.get(int(raw_id))
            except Exception:
                art = None
            if art:
                k = v28_2_final_event_key(art)
                # Trust best article key only when compatible with issue intent or no intent key exists.
                if k and (not keys or k in keys or v28_2_custom_event_key_from_text(intent_text) == k):
                    keys.add(k)
    return {k for k in keys if k}


def v28_2_distinct_anchor_sets(text):
    t = clean_html_text(text)
    entities = {clean_html_text(m.group(0)) for m in V28_CORE_ENTITY_RE.finditer(t)} if 'V28_CORE_ENTITY_RE' in globals() else set()
    tokens = set()
    for tok in tokenize_for_similarity(t):
        tok = clean_html_text(tok)
        if not tok or tok in V28_ANCHOR_STOPWORDS or tok in V28_2_GENERIC_EVENT_WORDS:
            continue
        if len(tok) < 2 or re.fullmatch(r"\d+", tok):
            continue
        tokens.add(tok)
    action_tokens = {tok for tok in tokens if re.search(r"파업|쟁의|성과급|지분|주식교환|정책협의|협의체|과징금|제재|유출|등급|데이터센터|GPU|엔비디아|앤트로픽|업무혁신|가이드|온플법|디지털자산", tok, re.I)}
    return entities, tokens, action_tokens


def v28_2_strict_replacement_match(article, issue, decision, requested_json_key, issue_keys=None):
    issue_keys = issue_keys or set()
    article_key = v28_2_final_event_key(article)
    if article_key and issue_keys and article_key in issue_keys:
        return True, f"same_final_event_key:{article_key}"
    # Allow platform enforcement base match even if a custom key was not available.
    try:
        art_enf = v27_platform_enforcement_key(article, include_stage=False)
        if art_enf and any(art_enf in k for k in issue_keys):
            return True, f"same_platform_enforcement_base:{art_enf}"
    except Exception:
        pass
    # If both sides have specific but different keys, do not use this article as replacement.
    if article_key and issue_keys:
        return False, f"different_event_key:{article_key}"

    issue_text = clean_html_text(" ".join(str(x or '') for x in [
        (issue or {}).get('issue_group'), (issue or {}).get('top_title'), (issue or {}).get('label_reasons'),
        (decision or {}).get('reason'), (decision or {}).get('category')
    ]))
    article_text = v28_2_base_text(article)
    i_ent, i_tok, i_act = v28_2_distinct_anchor_sets(issue_text)
    a_ent, a_tok, a_act = v28_2_distinct_anchor_sets(article_text[:1800])
    # Strict fallback: one real entity + one action/object token + at least two total non-generic token overlaps.
    if i_ent and a_ent and (i_ent & a_ent) and i_act and a_act and (i_act & a_act):
        if len((i_tok & a_tok) - V28_2_GENERIC_EVENT_WORDS) >= 2:
            return True, "strict_anchor_entity_action_match"
    return False, "no_strict_same_event_match"


_BASE_v28_global_replacement_candidate_ids_v28_2 = v28_global_replacement_candidate_ids

def v28_global_replacement_candidate_ids(issue, decision, article_by_id, requested_json_key, existing_ids):
    """v28.2: expand globally only for strict same-event candidates.
    Broad keyword/anchor overlap alone is no longer enough.
    """
    issue_keys = v28_2_issue_event_keys(issue, decision, article_by_id)
    existing = {int(x) for x in existing_ids if str(x).isdigit()}
    scored = []
    for art_id, art in article_by_id.items():
        try:
        ...
        if aid in existing:
            continue
        ok, reason = v28_2_strict_replacement_match(art, issue, decision, requested_json_key, issue_keys=issue_keys)
        if not ok:
            continue
        # Reuse v28 score after the strict gate; add exact event-key bonus to keep true alternatives first.
        try:
            s = v28_global_candidate_score(art, issue, decision, requested_json_key, set(), issue_keys)
        except Exception:
            s = 0
        art_key = v28_2_final_event_key(art)
        if art_key and art_key in issue_keys:
            s += 300
        if reason.startswith('strict_anchor'):
            s += 80
        if s >= 30:
            scored.append((s, aid))
    scored.sort(reverse=True)
    return [aid for _s, aid in scored[:20]]


_BASE_v24_representative_alignment_v28_2 = v24_representative_alignment

def v24_representative_alignment(issue, decision, report_item):
    action, reason = _BASE_v24_representative_alignment_v28_2(issue, decision, report_item)
    if action == 'fail':
        return action, reason
    issue_keys = v28_2_issue_event_keys(issue, decision, None)
    item_key = v28_2_final_event_key(report_item)
    if issue_keys and item_key and item_key not in issue_keys:
        # Prevent wrong replacements inside contaminated issue groups, e.g. dunamu issue -> Kakao strike article.
        return 'fail', f"v28_2_representative_event_mismatch:{item_key} not in {sorted(issue_keys)[:3]}"
    if issue_keys and not item_key:
        # If a specific issue has an identifiable key but the article has none, require the old alignment to be strong.
        if action == 'weak':
            return 'fail', f"v28_2_specific_issue_unkeyed_weak_article:{reason}"
    return action, reason


_BASE_v21_strict_cross_issue_duplicate_v28_2 = v21_strict_cross_issue_duplicate

def v28_2_duplicate_priority(item):
    key = v28_2_final_event_key(item)
    cat = item.get('카테고리') or item.get('JSON카테고리') or ''
    score = 0.0
    try:
        score += float(item.get('대표선택점수') or 0)
    except Exception:
        pass
    try:
        score += min(30, int(item.get('본문글자수') or 0) / 250)
    except Exception:
        pass
    if key.startswith('self_labor') or key.startswith('self_'):
        score += 80 if cat == JSON_KEYS_ORDER[0] else -35
    elif key.startswith('gov_') or key.startswith('v27_platform') or key.startswith('v15_privacy'):
        score += 70 if cat == JSON_KEYS_ORDER[1] else -10
    elif key.startswith('competitor') or key.startswith('overseas'):
        score += 55 if cat == JSON_KEYS_ORDER[2] else 0
    elif key.startswith('industry'):
        score += 50 if cat == JSON_KEYS_ORDER[3] else 0
    # Prefer major/clean press over obscure sources for same event.
    try:
        score += press_score(item.get('언론사', ''))
    except Exception:
        pass
    if item.get('본문추출방식') in {'critical_without_body', 'v28_1_bodyless_rejected'}:
        score -= 45
    return score


def v21_strict_cross_issue_duplicate(new_item, existing_items):
    new_key = v28_2_final_event_key(new_item)
    if new_key and not new_key.startswith('weak:'):
        for old in existing_items:
            old_key = v28_2_final_event_key(old)
            if old_key and old_key == new_key:
                return old, f"same_v28_2_final_event_key:{new_key}"
    return _BASE_v21_strict_cross_issue_duplicate_v28_2(new_item, existing_items)


_BASE_v21_remove_obvious_final_duplicates_v28_2 = v21_remove_obvious_final_duplicates

def v21_remove_obvious_final_duplicates(final_items):
    kept, removed = _BASE_v21_remove_obvious_final_duplicates_v28_2(final_items)
    by_key = {}
    for item in kept:
        key = v28_2_final_event_key(item)
        if not key or key.startswith('weak:'):
            by_key[id(item)] = item
            continue
        cur = by_key.get(key)
        if cur is None:
            by_key[key] = item
            continue
        if v28_2_duplicate_priority(item) > v28_2_duplicate_priority(cur) + 1:
            removed.append((cur, f"v28_2_final_event_duplicate_removed:{key}:winner={item.get('기사제목','')[:80]}"))
            by_key[key] = item
        else:
            removed.append((item, f"v28_2_final_event_duplicate_removed:{key}:winner={cur.get('기사제목','')[:80]}"))
    collapsed = list(by_key.values())
    try:
        collapsed = sorted(collapsed, key=v26_final_item_priority_score, reverse=True)
    except Exception:
        pass
    return collapsed, removed


_BASE_v20_gemini_edit_issues_v28_2 = v20_gemini_edit_issues
...
- Avoid duplicates aggressively. Select only one representative for the same event even if it appears in different sections.
- Kakao strike/labor articles are Self issues only; never put another Kakao strike representative into Government.
- 과기정통부-방미통위 policy council / AI-media-data-sharing articles are one Government issue only; choose one representative.
- If an issue representative fails body extraction, backup_article_ids must be from the same concrete event. Do not use broad keyword overlap such as Kakao/AI/government/platform alone.
- Same-event replacement must share the core entity plus the action/object. Examples: Kakao+strike, MSIT+방미통위+policy council, Naver+Nvidia+AI factory, OpenAI+data center.
- Do not fill Government with duplicated Self articles, event/expo notes, or single-company support-program PR.
- Do not fill Competitor/Overseas with generic logistics MOU/PoC unless a major Kakao competitor/global platform actor is central.
- Industry should prioritize structural AI infrastructure/data-center/GPU/cloud/security changes.
- Recommended final size: 10-13. It is better to return fewer clean issues than many duplicates.
