...
# =========================================================
# v34 overrides: stable labeling + strict government-to-competitor guardrail
# - Labeling model upgraded from flash-lite to flash by default.
# - Label batch size reduced to 35 and failed batches are retried in smaller chunks.
# - Domestic Korean government / National Assembly / regulator policy issues cannot remain
#   in Competitor/Overseas. They are reassigned to Government/National Assembly.
# - Exception: specific competitor-targeted enforcement/risk events such as Coupang
#   개인정보위/공정위 과징금, 자사우대 소송, etc. remain competitor risk.
# =========================================================

V6_VERSION = "google_rss_v34_flash_labeling_government_guardrail"

# 2안: labeling = flash, editor/QA = pro 유지. 환경변수로 명시한 경우에는 그 값을 우선한다.
GEMINI_MODEL_LABELING = os.getenv("GEMINI_MODEL_LABELING", "gemini-2.5-flash")
V20_LABEL_BATCH_SIZE = 35
try:
    V23_LABEL_SPLIT_BATCH_SIZE = min(int(V23_LABEL_SPLIT_BATCH_SIZE), 18)
except Exception:
    V23_LABEL_SPLIT_BATCH_SIZE = 18

# 국내 정부/국회/규제기관. '정부'만으로도 잡되, 해외정부 문맥은 별도 예외 처리한다.
V34_DOMESTIC_GOV_ACTOR_RE = re.compile(
    r"대한민국\s*정부|한국\s*정부|국내\s*정부|정부|국회|상임위|정무위|과방위|과학기술정보방송통신위원회|"
    r"과기정통부|과학기술정보통신부|방미통위|방송미디어통신위원회|방통위|개보위|개인정보보호위원회|개인정보위|"
    r"공정위|공정거래위원회|금융위|금융위원회|금감원|금융감독원|행안부|행정안전부|중기부|중소벤처기업부|"
    r"문체부|문화체육관광부|산업부|산업통상자원부|보건복지부|복지부|고용노동부|노동부|경찰청|검찰|KISA|인터넷진흥원|"
    r"정보통신산업진흥원|NIPA|한국거래소|신용보증기금|서울보증보험",
    re.IGNORECASE,
)

V34_FOREIGN_GOV_RE = re.compile(
    r"미국|EU|유럽연합|유럽|중국|일본|영국|프랑스|독일|캐나다|호주|인도|대만|싱가포르|브라질|"
    r"FTC|FCC|SEC|백악관|의회|상원|하원|집행위원회|European Commission|Commission|트럼프\s*행정부|바이든\s*행정부",
    re.IGNORECASE,
)

V34_GOV_POLICY_ACTION_RE = re.compile(
    r"정책|규제|법안|개정안|시행령|시행규칙|고시|가이드라인|행정예고|입법예고|발표|출범|착수|추진|선정|지원|"
    r"구축|조성|도입|확대|강화|협의체|협의회|간담회|업무협약|MOU|수여식|인증서|사업|프로젝트|보험|정책보험|"
    r"연구실|자율실험실|유니콘|인재양성|교육|공모|보조금|스케일업|지원사업|세미나|컨퍼런스|설명회|"
    r"과징금|제재|조사|시정명령|고발|행정소송|판결|처분",
    re.IGNORECASE,
)

V34_COMPANY_ACTOR_RE = re.compile(
    r"네이버|NAVER|쿠팡|Coupang|SKT|SK텔레콤|KT|케이티|LGU\+|LG유플러스|토스|비바리퍼블리카|"
    r"배달의민족|배민|우아한형제들|구글|Google|오픈AI|OpenAI|MS|마이크로소프트|Microsoft|메타|Meta|"
    r"애플|Apple|X\b|엑스\b|트위터|Twitter|앤트로픽|Anthropic|엔비디아|NVIDIA|라인야후|우버|Uber",
    re.IGNORECASE,
)

V34_COMPETITOR_ENFORCEMENT_RE = re.compile(
    r"과징금|과태료|제재|조사|시정명령|행정소송|소송|판결|고발|수사|처분|위반|불법|개인정보|정보\s*유출|"
    r"무단\s*수집|기만\s*광고|허위\s*광고|자사우대|최혜대우|인앱결제|등급\s*상향|청소년\s*이용불가|"
    r"앱마켓|플레이스토어|서비스\s*장애|해킹|보안|피싱",
    re.IGNORECASE,
)

V34_COMPETITOR_STRATEGY_RE = re.compile(
    r"투자|추가\s*투자|제휴|협력|파트너십|AI\s*팩토리|AI\s*펀드|데이터센터|클라우드|앤트로픽|엔비디아|"
    r"GPU|NPU|인수|합병|매각|지분|출시|개편|가격|요금|서비스|전략|인프라|상용화|수주|계약|MOU",
    re.IGNORECASE,
)

V34_PURE_GOV_TITLE_RE = re.compile(
    r"^\s*(?:\[[^\]]*정부[^\]]*\]\s*)?(정부|국회|과기정통부|방미통위|개보위|개인정보위|공정위|금융위|금감원|행안부|중기부|문체부|산업부|복지부|고용노동부|경찰청|KISA)\b",
    re.IGNORECASE,
)
...
def v34_text(obj):
    ...
    parts = []
    for k in [
        "기사제목", "top_title", "issue_group", "issue_family", "primary_category", "internal_category",
        "Gemini이슈그룹", "Gemini이슈유형", "Gemini내부카테고리", "Gemini카테고리",
        "본문요약", "RSS요약", "top_summary", "본문전문", "reason", "selection_reason", "Gemini판단사유",
    ]:
        v = obj.get(k, "") if isinstance(obj, dict) else ""
        if v:
            parts.append(str(v)[:2800] if k == "본문전문" else str(v))
    if isinstance(obj, dict):
        for k in ["article_titles", "candidate_titles", "candidate_article_ids", "article_ids"]:
            v = obj.get(k)
...
def v34_titleish(text):
    t = clean_html_text(text)
    # Title and issue-group usually appear at the front of v34_text().
    return t[:320]


def v34_has_domestic_gov(text):
    t = clean_html_text(text)
    if not t:
        return False
    return bool(V34_DOMESTIC_GOV_ACTOR_RE.search(t))


def v34_is_specific_competitor_targeted_event(text):
    """경쟁사 특정 제재/소송/운영 리스크는 경쟁사 섹션에 둘 수 있다."""
    t = clean_html_text(text)
    titleish = v34_titleish(t)
    if not t:
        return False
    try:
        if "v32_is_coupang_competitor_event" in globals() and v32_is_coupang_competitor_event(t):
            return True
    except Exception:
        pass
    # 기업명이 제목/핵심부에 있고 제재·소송·운영 리스크가 명확하면 경쟁사 리스크로 허용.
    if V34_COMPANY_ACTOR_RE.search(titleish) and V34_COMPETITOR_ENFORCEMENT_RE.search(t):
        return True
    # 기존 경쟁사 집행 판단이 있으면 사용.
    try:
        if "v31_is_specific_competitor_public_enforcement" in globals() and v31_is_specific_competitor_public_enforcement(t):
            return True
    except Exception:
        pass
    return False
...
def v34_is_competitor_strategy_or_global(text):
    t = clean_html_text(text)
    titleish = v34_titleish(t)
    if not t:
        return False
    if V34_COMPANY_ACTOR_RE.search(titleish) and V34_COMPETITOR_STRATEGY_RE.search(t):
        return True
    try:
        if "v32_is_naver_telco_strategy" in globals() and v32_is_naver_telco_strategy(t):
            return True
        if "v32_is_global_platform_or_overseas_issue" in globals() and v32_is_global_platform_or_overseas_issue(t):
            return True
    except Exception:
        pass
    return False
...
def v34_is_domestic_government_policy_only(text):
    """국내 정부·국회·규제기관이 주체인 일반 정책/사업/협의체 기사인지 판단한다."""
    t = clean_html_text(text)
    if not t:
        return False
    titleish = v34_titleish(t)
    # 해외 정부/규제만 주체인 경우는 경쟁사/해외 이슈가 될 수 있으므로 국내 정부 정책으로 보지 않는다.
    if V34_FOREIGN_GOV_RE.search(titleish) and not V34_DOMESTIC_GOV_ACTOR_RE.search(titleish):
        return False
    if v34_is_specific_competitor_targeted_event(t):
        return False
    # 제목 핵심부가 경쟁사 전략 기사면 본문에 정부 프로젝트가 언급돼도 바로 정부 전용으로 보지 않는다.
    if V34_COMPANY_ACTOR_RE.search(titleish) and V34_COMPETITOR_STRATEGY_RE.search(titleish):
        return False
    # Gemini/internal category가 GOV 계열이면 정부 후보로 존중한다.
    if re.search(r"\bGOV_|정부_국회|정부/국회|정부정책|정부\s*정책", t):
        if not v34_is_specific_competitor_targeted_event(t):
            return True
    if V34_DOMESTIC_GOV_ACTOR_RE.search(t) and V34_GOV_POLICY_ACTION_RE.search(t):
        # 정책보험/지원사업/연구실/협의체/선정/구축류는 경쟁사 섹션에 들어가면 거의 항상 오분류다.
        if re.search(r"정책보험|지원사업|유니콘|연구실|자율실험실|협의체|협의회|선정|수여식|인증서|구축|조성|교육|인재양성|공모", t, re.IGNORECASE):
            return True
        # 제목이 정부기관으로 시작하면 정부 주체 기사로 본다.
        if V34_PURE_GOV_TITLE_RE.search(titleish):
            return True
        # 경쟁사 기업명이 제목에 없고 국내 정부기관이 정책 액션을 하는 경우.
        if not V34_COMPANY_ACTOR_RE.search(titleish):
            return True
    return False


def v34_output_category_for_text(text, fallback="산업동향"):
    fallback = v19_normalize_category_key(fallback, fallback="산업동향") if "v19_normalize_category_key" in globals() else fallback
    if v34_is_domestic_government_policy_only(text):
        return JSON_KEYS_ORDER[1]
    return fallback


def v34_adjust_label_category(label, article):
    if not isinstance(label, dict):
        return label
    text = v34_text(article)
    # include label fields too, because Gemini may provide GOV category even if article text is short.
    text = clean_html_text(" ".join([
        text,
        str(label.get("primary_category", "")),
        str(label.get("internal_category", "")),
        str(label.get("issue_family", "")),
        str(label.get("issue_group", "")),
        str(label.get("reason", "")),
        str(label.get("gov_tier", "")),
    ]))
    primary = v19_normalize_category_key(label.get("primary_category") or article.get("JSON카테고리") or "산업동향", fallback="산업동향")
    internal = str(label.get("internal_category", ""))
    gov_hint = internal.startswith("GOV_") or str(label.get("gov_tier", "")).startswith("GOV_")
    if (primary == JSON_KEYS_ORDER[2] or gov_hint) and v34_is_domestic_government_policy_only(text):
        label["primary_category"] = JSON_KEYS_ORDER[1]
        if not str(label.get("internal_category", "")).startswith("GOV_"):
            if re.search(r"AI|인공지능|저작권|데이터센터|클라우드|연구실|유니콘", text, re.IGNORECASE):
                label["internal_category"] = "GOV_AI_DIGITAL_POLICY"
            elif re.search(r"스테이블코인|디지털자산|가상자산|금융|금감원|금융위|FIU", text, re.IGNORECASE):
                label["internal_category"] = "GOV_FINANCIAL_DIGITAL_POLICY"
            else:
                label["internal_category"] = "GOV_PLATFORM_REGULATION"
        label["reason"] = v20_clip((str(label.get("reason", "")) + " / v34_domestic_government_policy_forced_to_gov").strip(" /"), 260)
    return label


# Labeling: use smaller batches and partial split fallback, so one bad JSON batch does not demote many candidates.
def v34_local_label(article):
    local = v20_local_article_label(article)
    local["label_source"] = "local_v34_fallback"
    return v34_adjust_label_category(local, article)


def v34_try_label_batch(client, batch, recent_past_text, label_tag=""):
    labels = v23_label_one_batch(client, batch, recent_past_text, label_tag) if "v23_label_one_batch" in globals() else _BASE_v20_gemini_label_candidates_v23(client, batch, recent_past_text)
    for art_id, label in list(labels.items()):
        article = {int(a.get("id")): a for a in batch if str(a.get("id", "")).isdigit()}.get(int(art_id), {})
        labels[int(art_id)] = v34_adjust_label_category(label, article)
    return labels
...
def v20_gemini_label_candidates(client, candidates, recent_past_text):
    ...
    if not client:
        raise RuntimeError("Gemini client 없음")
    try:
        V22_LABELING_LOCAL_BATCHES
    except NameError:
        V22_LABELING_LOCAL_BATCHES = 0
    try:
        V22_LABELING_RETRY_BATCHES
    except NameError:
        V22_LABELING_RETRY_BATCHES = 0
    ...
    for batch_idx, batch in enumerate(batches, 1):
        try:
            labels = v34_try_label_batch(client, batch, recent_past_text, f"v34_batch{batch_idx}")
            all_labels.update(labels)
            print(f"  └ 🏷️ v34 라벨링 배치 {batch_idx}/{len(batches)} 완료: Gemini {len(labels)}개 / coverage {len(labels)/max(1,len(batch)):.0%}")
            continue
        except Exception as first_error:
            V22_LABELING_RETRY_BATCHES += 1
            try:
                time.sleep(0.8 + random.uniform(0, 0.8))
                labels = v34_try_label_batch(client, batch, recent_past_text, f"v34_retry{batch_idx}")
                all_labels.update(labels)
                print(f"  └ 🏷️ v34 라벨링 배치 {batch_idx}/{len(batches)} 재시도 성공: Gemini {len(labels)}개")
                continue
            except Exception:
                pass

            print(f"  └ ⚠️ v34 라벨링 배치 {batch_idx}/{len(batches)} 실패. 더 작은 단위로 분할 재시도: {first_error}")
            # Split into smaller chunks, then fallback only failed chunks.
            for sub_idx, start in enumerate(range(0, len(batch), V23_LABEL_SPLIT_BATCH_SIZE), 1):
                sub = batch[start:start + V23_LABEL_SPLIT_BATCH_SIZE]
                try:
                    V22_LABELING_RETRY_BATCHES += 1
                    labels = v34_try_label_batch(client, sub, recent_past_text, f"v34_split{batch_idx}-{sub_idx}")
                    all_labels.update(labels)
                    print(f"     └ ✅ split {batch_idx}-{sub_idx}: Gemini {len(labels)}개")
                except Exception as sub_error:
                    # Last resort: split even smaller once. Only the failing mini chunk becomes local.
                    mini_size = max(6, min(9, len(sub)))
                    for mini_idx, mstart in enumerate(range(0, len(sub), mini_size), 1):
                        mini = sub[mstart:mstart + mini_size]
                        try:
                            V22_LABELING_RETRY_BATCHES += 1
                            labels = v34_try_label_batch(client, mini, recent_past_text, f"v34_mini{batch_idx}-{sub_idx}-{mini_idx}")
                            all_labels.update(labels)
                            print(f"        └ ✅ mini {batch_idx}-{sub_idx}-{mini_idx}: Gemini {len(labels)}개")
                        except Exception:
                            V22_LABELING_LOCAL_BATCHES += 1
                            print(f"        └ ⚠️ mini {batch_idx}-{sub_idx}-{mini_idx} 실패. 이 {len(mini)}개만 로컬 라벨 사용: {type(sub_error).__name__}")
                            for article in mini:
                                all_labels[int(article["id"])] = v34_local_label(article)

    # Ensure full coverage and apply the government guardrail to every label.
    for article in candidates:
        art_id = int(article["id"])
        if art_id not in all_labels:
            all_labels[art_id] = v34_local_label(article)
        else:
            all_labels[art_id] = v34_adjust_label_category(all_labels[art_id], article)
    return all_labels


_BASE_v20_normalize_label_json_v34 = v20_normalize_label_json

def v20_normalize_label_json(data, valid_ids, article_by_id):
    labels = _BASE_v20_normalize_label_json_v34(data, valid_ids, article_by_id)
    for art_id, label in list(labels.items()):
        article = article_by_id.get(int(art_id), {})
        labels[int(art_id)] = v34_adjust_label_category(label, article)
    return labels
...
_BASE_v20_issue_to_output_category_v34 = v20_issue_to_output_category

def v20_issue_to_output_category(issue):
    base = _BASE_v20_issue_to_output_category_v34(issue)
    text = v34_text(issue)
    if base == JSON_KEYS_ORDER[2] and v34_is_domestic_government_policy_only(text):
        return JSON_KEYS_ORDER[1]
    # If issue itself is clearly GOV-family, never surface as competitor unless it is a specific competitor-targeted enforcement/risk.
    if re.search(r"\bGOV_|government_|정부_국회|정부/국회", text, re.IGNORECASE) and v34_is_domestic_government_policy_only(text):
        return JSON_KEYS_ORDER[1]
    return base


_BASE_v20_normalize_issue_editor_json_v34 = v20_normalize_issue_editor_json

def v20_normalize_issue_editor_json(data, issue_by_id, article_by_id):
    decisions, backups = _BASE_v20_normalize_issue_editor_json_v34(data, issue_by_id, article_by_id)
    def fix_decision(d):
        issue = issue_by_id.get(d.get("issue_id"), {})
        text = clean_html_text(" ".join([v34_text(issue), str(d.get("category", "")), str(d.get("reason", ""))]))
        cat = v19_normalize_category_key(d.get("category"), fallback=v20_issue_to_output_category(issue))
        if cat == JSON_KEYS_ORDER[2] and v34_is_domestic_government_policy_only(text):
            d["category"] = JSON_KEYS_ORDER[1]
            d["v34_category_repair"] = "competitor_to_government_before_processing"
            d["reason"] = v20_clip((str(d.get("reason", "")) + " / v34_competitor_to_gov").strip(" /"), 240)
        return d
    decisions = [fix_decision(dict(d)) for d in decisions]
    backups = [fix_decision(dict(d)) for d in backups]
    return decisions, backups


_BASE_v23_final_category_guardrail_v34 = v23_final_category_guardrail

def v23_final_category_guardrail(item, requested_category):
    requested = v19_normalize_category_key(requested_category, fallback=JSON_KEYS_ORDER[3])
    text = v34_text(item)
    if requested == JSON_KEYS_ORDER[2] and v34_is_domestic_government_policy_only(text):
        return "reassign", JSON_KEYS_ORDER[1], "v34_domestic_government_policy_not_competitor"
    return _BASE_v23_final_category_guardrail_v34(item, requested_category)


_BASE_is_report_item_relevant_v34 = is_report_item_relevant

def is_report_item_relevant(report_item, json_key):
    requested = v19_normalize_category_key(json_key, fallback=JSON_KEYS_ORDER[3])
    text = v34_text(report_item)
    if requested == JSON_KEYS_ORDER[2] and v34_is_domestic_government_policy_only(text):
        return False, "v34_domestic_government_policy_should_be_government"
    return _BASE_is_report_item_relevant_v34(report_item, json_key)


_BASE_v32_competitor_candidate_text_v34 = v32_competitor_candidate_text

def v32_competitor_candidate_text(text):
    t = clean_html_text(text)
    if v34_is_domestic_government_policy_only(t):
        return False, "v34_domestic_government_policy_to_government"
    return _BASE_v32_competitor_candidate_text_v34(text)


_BASE_v32_competitor_issue_eligible_v34 = v32_competitor_issue_eligible

def v32_competitor_issue_eligible(issue):
    text = v34_text(issue)
    if v34_is_domestic_government_policy_only(text):
        issue["v34_competitor_block_reason"] = "domestic_government_policy"
        return False
    return _BASE_v32_competitor_issue_eligible_v34(issue)


_BASE_v32_competitor_article_eligible_v34 = v32_competitor_article_eligible

def v32_competitor_article_eligible(article):
    text = v34_text(article)
    if v34_is_domestic_government_policy_only(text):
        article["v34_competitor_block_reason"] = "domestic_government_policy"
        return False
    return _BASE_v32_competitor_article_eligible_v34(article)


def v34_add_skip(skip_rows, item, reason):
    try:
    ...
    if skip_rows is not None:
        skip_rows.append({
            ...
            "토큰점수": "",
            "본문점수": "",
            "제외단계": "v34_final_repair",
        })


def v34_set_category(item, cat, reason=""):
    try:
        v32_set_category(item, cat, reason)
    except Exception:
        item["카테고리"] = cat
        item["카테고리명"] = JSON_KEY_TO_DISPLAY.get(cat, cat)
    item["v34_category_repair"] = reason
    return item


def v34_repair_competitor_government_leaks(final_report_data, skip_rows=None):
    for item in list(final_report_data):
        cat = item.get("카테고리") or ""
        text = v34_text(item)
        if cat == JSON_KEYS_ORDER[2] and v34_is_domestic_government_policy_only(text):
            v34_set_category(item, JSON_KEYS_ORDER[1], "v34_competitor_government_policy_to_gov")
            v34_add_skip(skip_rows, item, "v34_reassigned_competitor_government_policy_to_gov")

    # Remove pure government duplicates after moving them to government.
    seen = {}
    cleaned = []
    for item in list(final_report_data):
        cat = item.get("카테고리") or ""
        key = ""
        try:
            key = v33_public_policy_key(item) if cat == JSON_KEYS_ORDER[1] and "v33_public_policy_key" in globals() else ""
        except Exception:
            key = ""
        if not key and cat == JSON_KEYS_ORDER[1] and v34_is_domestic_government_policy_only(v34_text(item)):
            key = title_fingerprint(item.get("기사제목", ""))[:80]
        if key:
            old = seen.get(key)
            if old is not None:
                try:
                    old_score = v29_item_priority_score(old)
                    new_score = v29_item_priority_score(item)
                except Exception:
                    old_score = v20_float(old.get("대표선택점수"), 0)
                    new_score = v20_float(item.get("대표선택점수"), 0)
                if new_score > old_score:
                    v34_add_skip(skip_rows, old, f"v34_duplicate_gov_policy_removed:{key}")
                    cleaned = [x for x in cleaned if x is not old]
                    seen[key] = item
                    cleaned.append(item)
                else:
                    v34_add_skip(skip_rows, item, f"v34_duplicate_gov_policy_removed:{key}")
                continue
            seen[key] = item
        cleaned.append(item)
    final_report_data[:] = cleaned

    # Reorder and respect category caps. If government exceeds max after forced moves, drop weakest gov items.
    capped = []
    for cat in JSON_KEYS_ORDER:
        bucket = [x for x in final_report_data if x.get("카테고리") == cat]
        try:
            bucket = sorted(bucket, key=v29_item_priority_score, reverse=True)
        except Exception:
            bucket = sorted(bucket, key=lambda x: v20_float(x.get("대표선택점수"), 0), reverse=True)
        max_keep = CATEGORY_MAX.get(cat, 99)
        capped.extend(bucket[:max_keep])
        for item in bucket[max_keep:]:
            v34_add_skip(skip_rows, item, f"v34_category_cap_removed_after_gov_repair:{cat}:{max_keep}")
    order_base = {key: idx * 1000 for idx, key in enumerate(JSON_KEYS_ORDER, 1)}
    ordered = []
    for cat in JSON_KEYS_ORDER:
        bucket = [x for x in capped if x.get("카테고리") == cat]
        ...
        for idx, item in enumerate(bucket, 1):
            item["선정순서"] = order_base.get(cat, 9000) + idx
            item["v34_final_priority"] = v20_float(item.get("대표선택점수"), 0)
            ordered.append(item)
    final_report_data[:] = ordered
    return final_report_data


_BASE_v32_repair_final_items_v34 = v32_repair_final_items

def v32_repair_final_items(final_report_data, skip_rows=None):
    try:
        _BASE_v32_repair_final_items_v34(final_report_data, skip_rows)
    except Exception:
        pass
    return v34_repair_competitor_government_leaks(final_report_data, skip_rows)


# v33 uses v33_repair_final_items directly in a few places. Override it as well if present.
try:
    _BASE_v33_repair_final_items_v34 = v33_repair_final_items
    def v33_repair_final_items(final_report_data, skip_rows=None):
        try:
            _BASE_v33_repair_final_items_v34(final_report_data, skip_rows)
        except Exception:
            pass
        return v34_repair_competitor_government_leaks(final_report_data, skip_rows)
except Exception:
...
_BASE_v20_process_issue_v34 = v20_process_issue

def v20_process_issue(decision, issue_by_id, article_by_id, recent_past_items, final_report_data, body_failed_rows, skip_rows, processed_article_ids, order_counter):
    issue = issue_by_id.get(decision.get("issue_id"), {}) if isinstance(issue_by_id, dict) else {}
    if issue:
        text = clean_html_text(" ".join([v34_text(issue), str(decision.get("category", "")), str(decision.get("reason", ""))]))
        requested = v19_normalize_category_key(decision.get("category"), fallback=v20_issue_to_output_category(issue))
        if requested == JSON_KEYS_ORDER[2] and v34_is_domestic_government_policy_only(text):
            decision = dict(decision)
            decision["category"] = JSON_KEYS_ORDER[1]
            decision["v34_category_repair"] = "process_issue_competitor_to_gov"
    item, order_counter, status = _BASE_v20_process_issue_v34(decision, issue_by_id, article_by_id, recent_past_items, final_report_data, body_failed_rows, skip_rows, processed_article_ids, order_counter)
    if item and item.get("카테고리") == JSON_KEYS_ORDER[2] and v34_is_domestic_government_policy_only(v34_text(item)):
        v34_set_category(item, JSON_KEYS_ORDER[1], "v34_post_process_competitor_to_gov")
        v34_add_skip(skip_rows, item, "v34_post_process_reassigned_to_gov")
    return item, order_counter, status


_BASE_v26_1_refill_after_final_repair_v34 = v26_1_refill_after_final_repair

def v26_1_refill_after_final_repair(decisions, backup_decisions, issues, issue_by_id, article_by_id, recent_past_items, final_report_data, body_failed_rows, skip_rows, processed_article_ids, order_counter, status_counts):
    order_counter = _BASE_v26_1_refill_after_final_repair_v34(
        decisions=decisions,
        backup_decisions=backup_decisions,
        issues=issues,
        issue_by_id=issue_by_id,
    ...
    v34_repair_competitor_government_leaks(final_report_data, skip_rows)
    return order_counter


_BASE_v21_remove_obvious_final_duplicates_v34 = v21_remove_obvious_final_duplicates

def v21_remove_obvious_final_duplicates(final_items):
    kept, removed = _BASE_v21_remove_obvious_final_duplicates_v34(final_items)
    skip_like = []
    v34_repair_competitor_government_leaks(kept, skip_like)
    for row in skip_like:
        removed.append(({"기사제목": row.get("후보제목", ""), "링크": row.get("후보링크", "")}, row.get("중복판정이유", "v34_repair")))
    return kept, removed


_BASE_v20_gemini_quality_check_v34 = v20_gemini_quality_check

def v20_gemini_quality_check(client, final_report_data, final_briefing_text):
    try:
        skip_like = []
        v34_repair_competitor_government_leaks(final_report_data, skip_like)
    except Exception:
        pass
    qa = _BASE_v20_gemini_quality_check_v34(client, final_report_data, final_briefing_text)
    try:
        data = v22_parse_qa_json(qa) if qa else {"overall": "양호", "warnings": [], "suggested_human_checks": []}
    except Exception:
        data = {"overall": "주의", "warnings": [clean_html_text(qa)[:500]], "suggested_human_checks": []}
    warnings = data.get("warnings") if isinstance(data.get("warnings"), list) else []
    checks = data.get("suggested_human_checks") if isinstance(data.get("suggested_human_checks"), list) else []
    leak_titles = [x.get("기사제목", "") for x in final_report_data if x.get("카테고리") == JSON_KEYS_ORDER[2] and v34_is_domestic_government_policy_only(v34_text(x))]
    if leak_titles:
        msg = "경쟁사/해외이슈에 국내 정부 정책 후보가 남아 있음: " + "; ".join(t[:60] for t in leak_titles[:3])
        warnings.append(msg)
        checks.append(msg)
        data["overall"] = "문제"
    data["warnings"] = warnings
...
# =========================================================
# v35 overrides: title/link-only fallback + final section sanity repair
# - If every article inside a Gemini-selected issue fails body extraction,
#   keep the best article's title/source/link instead of replacing the issue
#   with a weaker refill candidate.
# - Re-check final section placement after all refill/repair steps.
# - Competitor/overseas is limited to core competitors/global platforms or
#   specific competitor-targeted enforcement/risk events.
# =========================================================

V6_VERSION = "google_rss_v35_title_link_fallback_section_repair"

# Bodyless fallback should be used for editorially selected/high-priority issues,
# not for weak local filler.
V35_BODYLESS_MIN_PRIORITY = 4
V35_BODYLESS_MIN_ISSUE_SCORE = 70

V35_CORE_COMPETITOR_RE = re.compile(
    r"네이버|NAVER|SKT|SK텔레콤|KT\b|\bKT\b|케이티|LGU\+|LG유플러스|LG\s*U\+|"
    r"쿠팡|Coupang|토스|비바리퍼블리카|배달의민족|배민|우아한형제들|"
    r"구글|Google|오픈AI|OpenAI|챗GPT|ChatGPT|MS\b|마이크로소프트|Microsoft|"
    r"메타|Meta|애플|Apple|앤트로픽|Anthropic|엔비디아|NVIDIA|라인야후|우버|Uber|"
    r"스페이스X|SpaceX|X\b|엑스\b|트위터|Twitter",
    re.IGNORECASE,
)

V35_COMPETITOR_MATERIAL_RE = re.compile(
    r"AI|인공지능|LLM|생성형|에이전트|AI\s*팩토리|데이터센터|클라우드|GPU|NPU|반도체|"
    r"제휴|협력|파트너십|동맹|투자|추가\s*투자|펀드|인수|합병|매각|지분|상장|IPO|"
    r"출시|개편|요금|가격|광고|검색|커머스|플랫폼|수수료|정산|보안|양자|PQC|"
    r"개인정보|유출|해킹|장애|먹통|피싱|소송|판결|과징금|제재|조사|규제|시정명령|"
    r"자사우대|최혜대우|인앱결제|앱마켓|등급\s*상향|청소년\s*이용불가|방한|회동|방문",
    re.IGNORECASE,
)

V35_WEAK_OFFTOPIC_RE = re.compile(
    r"비스토스|씨유메디칼|생체신호|의료진단기기|주식등의\s*수|대량보유|지분율\s*\d|"
    r"LG\s*CNS\s*[-–]\s*LX판토스|LX판토스|메가와이즈\s*청라|차세대\s*스마트물류|"
    r"휴머노이드\s*로봇\s*도입|물류센터|TDL\s*랩|부산\s*클라우드\s*데이|"
    r"브랜드평판|브랜드\s*순위|행사\s*개최|세미나\s*개최|컨퍼런스\s*개최|교육\s*실시",
    re.IGNORECASE,
)

V35_DOMESTIC_GOV_STRONG_TITLE_RE = re.compile(
    r"^\s*(정부|국회|과기정통부|과학기술정보통신부|방미통위|방송미디어통신위원회|방통위|"
    r"개보위|개인정보위|개인정보보호위원회|공정위|공정거래위원회|금융위|금융위원회|"
    r"금감원|금융감독원|행안부|중기부|중소벤처기업부|문체부|문화체육관광부|산업부|"
    r"고용노동부|노동부|경찰청|검찰|KISA|인터넷진흥원)\b",
    re.IGNORECASE,
)

V35_GOV_PROJECT_RE = re.compile(
    r"정책보험|지원사업|유니콘|연구실|자율실험실|협의체|협의회|선정|수여식|인증서|"
    r"구축|조성|교육|인재양성|공모|간담회|설명회|시행|적용|의무화|가이드라인|법안|시행령",
    re.IGNORECASE,
)
...
def v35_text(obj):
    try:
        ...
        if isinstance(obj, dict):
            return clean_html_text(" ".join(str(obj.get(k, "")) for k in [
                "기사제목", "issue_group", "issue_family", "internal_category", "primary_category",
                "Gemini이슈그룹", "Gemini이슈유형", "본문요약", "RSS요약", "본문전문", "reason"
            ]))
        return clean_html_text(obj)


def v35_titleish(text):
    try:
        return v34_titleish(text)
    except Exception:
        return clean_html_text(text)[:320]


def v35_has_core_competitor(text):
    return bool(V35_CORE_COMPETITOR_RE.search(clean_html_text(text)))


def v35_is_weak_offtopic(text):
    t = clean_html_text(text)
    titleish = v35_titleish(t)
    if not t:
        return True
    if V35_WEAK_OFFTOPIC_RE.search(titleish):
        # 단순 행사/세미나/컨퍼런스/클라우드 데이는 핵심 경쟁사명이 있어도 브리핑 우선순위가 낮다.
        if re.search(r"부산\s*클라우드\s*데이|세미나|컨퍼런스|행사\s*개최|교육\s*실시", titleish, re.IGNORECASE):
            if not re.search(r"대규모\s*투자|전략적\s*제휴|인수|합병|과징금|제재|소송|개인정보|장애|보안사고", t, re.IGNORECASE):
                return True
        # 네이버 AI팩토리/쿠팡 제재처럼 핵심 경쟁사 전략·리스크가 명확한 경우는 예외.
        if re.search(r"네이버|오픈AI|앤트로픽|구글|SKT|KT|LGU\+|쿠팡", titleish, re.IGNORECASE) and re.search(r"AI\s*팩토리|대규모\s*AI|데이터센터|양자보안|PQC|개인정보|과징금|제재|소송|투자|협력", t, re.IGNORECASE):
            return False
        return True
    # 공시/지분율 기사인데 핵심 플랫폼 경쟁사가 없으면 제외.
    if re.search(r"공시|주식등의\s*수|대량보유|지분율", titleish) and not v35_has_core_competitor(titleish):
        return True
    # 순수 투자/시황 노이즈는 제외. 단, 네이버 AI팩토리 등 실제 사업 이벤트는 하위 함수가 살린다.
    try:
        if v14_is_pure_market_or_stock_noise(t) and not re.search(r"AI\s*팩토리|데이터센터|투자\s*계획|제휴|협력|과징금|제재|소송|개인정보", t, re.IGNORECASE):
            return True
    except Exception:
        pass
    return False
...
def v35_is_specific_competitor_event(text):
    t = clean_html_text(text)
    if not t:
        return False
    try:
        ...
        pass
    titleish = v35_titleish(t)
    return bool(v35_has_core_competitor(titleish) and re.search(
        r"과징금|과태료|제재|조사|시정명령|행정소송|소송|판결|고발|수사|처분|위반|개인정보|유출|해킹|장애|자사우대|최혜대우|기만\s*광고|등급\s*상향|청소년\s*이용불가",
        t,
        re.IGNORECASE,
    ))


def v35_is_domestic_gov_main_actor(text):
    t = clean_html_text(text)
    if not t:
        return False
    titleish = v35_titleish(t)
    try:
        # Foreign-only government/regulator issues are not Korean Gov/National Assembly.
        if V34_FOREIGN_GOV_RE.search(titleish) and not V34_DOMESTIC_GOV_ACTOR_RE.search(titleish):
            return False
    except Exception:
        pass
    # Specific competitor-targeted enforcement is treated as competitor risk, even if a regulator appears.
    if v35_is_specific_competitor_event(t):
        return False
    if V35_DOMESTIC_GOV_STRONG_TITLE_RE.search(titleish):
        return True
    try:
        if v20_is_platform_obligation_text(t) and V34_DOMESTIC_GOV_ACTOR_RE.search(t) and V34_GOV_POLICY_ACTION_RE.search(t):
            return True
    except Exception:
        pass
    # Government project/support/committee stories are Government only when no core competitor is the title subject.
    try:
        if V34_DOMESTIC_GOV_ACTOR_RE.search(t) and V34_GOV_POLICY_ACTION_RE.search(t):
            if not V35_CORE_COMPETITOR_RE.search(titleish) and V35_GOV_PROJECT_RE.search(t):
                return True
            if re.search(r"\bGOV_|정부_국회|정부/국회", t, re.IGNORECASE) and not V35_CORE_COMPETITOR_RE.search(titleish):
                return True
    except Exception:
        pass
    return False
...
def v35_is_competitor_strategy_or_global(text):
    t = clean_html_text(text)
    if not t:
        return False
    titleish = v35_titleish(t)
    if v35_is_weak_offtopic(t):
        return False
    if v35_is_specific_competitor_event(t):
        return True
    # If the Korean government is the main actor, keep it in Government unless it is a specific competitor enforcement event.
    if v35_is_domestic_gov_main_actor(t):
        return False
    if V35_CORE_COMPETITOR_RE.search(titleish) and V35_COMPETITOR_MATERIAL_RE.search(t):
        return True
    # Some issue groups put the company in group/body rather than title. Allow only when the material signal is strong.
    if V35_CORE_COMPETITOR_RE.search(t) and re.search(r"AI\s*팩토리|앤트로픽|오픈AI|엔비디아|데이터센터|AI\s*펀드|개인정보|과징금|제재|소송|자사우대|양자보안|PQC|AI\s*인프라", t, re.IGNORECASE):
        return True
    return False


def v35_is_government_eligible(text):
    return v35_is_domestic_gov_main_actor(text)


def v35_is_industry_eligible(text):
    t = clean_html_text(text)
    if not t or v35_is_specific_competitor_event(t) or v35_is_domestic_gov_main_actor(t):
        return False
    if v35_is_competitor_strategy_or_global(t):
        return False
    if v35_is_weak_offtopic(t):
        return False
    return bool(re.search(r"AI|인공지능|데이터센터|GPU|NPU|클라우드|반도체|HBM|전력|플랫폼|스테이블코인|디지털자산|보안|해킹|딥페이크|저작권|알고리즘", t, re.IGNORECASE))


def v35_preferred_category_for_text(text, fallback="산업동향"):
    fallback = v19_normalize_category_key(fallback, fallback="산업동향") if "v19_normalize_category_key" in globals() else fallback
    t = clean_html_text(text)
    # Competitor-specific enforcement and competitor/global strategy must not be pushed into Industry/Gov by incidental wording.
    if v35_is_competitor_strategy_or_global(t):
        return JSON_KEYS_ORDER[2]
    if v35_is_government_eligible(t):
        return JSON_KEYS_ORDER[1]
    if v35_is_industry_eligible(t):
        return JSON_KEYS_ORDER[3]
    return fallback if fallback in JSON_KEYS_ORDER else JSON_KEYS_ORDER[3]


def v35_set_category(item, cat, reason=""):
    try:
        v34_set_category(item, cat, reason)
    except Exception:
        item["카테고리"] = cat
        item["카테고리명"] = JSON_KEY_TO_DISPLAY.get(cat, cat)
    item["v35_category_repair"] = reason
    return item


def v35_add_skip(skip_rows, item, reason):
    try:
        v34_add_skip(skip_rows, item, reason)
    except Exception:
        if skip_rows is not None:
            skip_rows.append({
                "검색어": item.get("검색어", ""),
                ...
                "후보언론사": item.get("언론사", ""),
                "중복판정이유": reason,
                "제외단계": "v35_final_repair",
            })


def v35_repair_final_sections(final_report_data, skip_rows=None):
    cleaned = []
    for item in list(final_report_data):
        text = v35_text(item)
        old_cat = item.get("카테고리") or ""
        preferred = v35_preferred_category_for_text(text, fallback=old_cat or "산업동향")

        # Competitor section must be a real core competitor/global platform or specific competitor risk.
        if old_cat == JSON_KEYS_ORDER[2] and not v35_is_competitor_strategy_or_global(text):
            if v35_is_government_eligible(text):
                v35_set_category(item, JSON_KEYS_ORDER[1], "v35_competitor_to_gov_main_actor")
                cleaned.append(item)
            elif v35_is_industry_eligible(text):
                v35_set_category(item, JSON_KEYS_ORDER[3], "v35_competitor_to_industry_structural")
                cleaned.append(item)
            else:
                v35_add_skip(skip_rows, item, "v35_removed_weak_competitor_or_offtopic")
            continue

        # Government section should not contain primarily competitor/global strategy stories.
        if old_cat == JSON_KEYS_ORDER[1] and preferred == JSON_KEYS_ORDER[2]:
            v35_set_category(item, JSON_KEYS_ORDER[2], "v35_gov_to_competitor_strategy")
            cleaned.append(item)
            continue

        # Industry should not absorb competitor enforcement/strategy or government policy.
        if old_cat == JSON_KEYS_ORDER[3] and preferred in {JSON_KEYS_ORDER[1], JSON_KEYS_ORDER[2]}:
            v35_set_category(item, preferred, "v35_industry_to_specific_section")
            cleaned.append(item)
            continue

        # If the item was assigned to a weak/off-topic section and has no valid preferred category, drop it.
        if v35_is_weak_offtopic(text) and old_cat == JSON_KEYS_ORDER[2]:
            v35_add_skip(skip_rows, item, "v35_removed_weak_offtopic_competitor")
            continue

        cleaned.append(item)

    # Deduplicate obvious same titles after moving sections.
    seen = set()
    deduped = []
    for item in cleaned:
        key = (item.get("카테고리", ""), title_fingerprint(item.get("기사제목", ""))[:100])
        if key[1] and key in seen:
            v35_add_skip(skip_rows, item, f"v35_duplicate_after_section_repair:{key[1]}")
            continue
        seen.add(key)
        deduped.append(item)

    # Respect category max and order by category. Bodyless title/link-only items keep their issue score via 대표선택점수.
    ordered = []
    order_base = {key: idx * 1000 for idx, key in enumerate(JSON_KEYS_ORDER, 1)}
    for cat in JSON_KEYS_ORDER:
        bucket = [x for x in deduped if x.get("카테고리") == cat]
        try:
            bucket = sorted(bucket, key=v29_item_priority_score, reverse=True)
        except Exception:
            bucket = sorted(bucket, key=lambda x: (v20_float(x.get("대표선택점수"), 0), v20_float(x.get("랭킹점수"), 0)), reverse=True)
        max_keep = CATEGORY_MAX.get(cat, 99)
        for idx, item in enumerate(bucket[:max_keep], 1):
            item["선정순서"] = order_base.get(cat, 9000) + idx
            ordered.append(item)
        for item in bucket[max_keep:]:
            v35_add_skip(skip_rows, item, f"v35_category_cap_removed:{cat}:{max_keep}")
    final_report_data[:] = ordered
    return final_report_data


def v35_candidate_ids(decision, issue):
    ids = []
    try:
        if decision.get("best_article_id"):
            ids.append(int(decision.get("best_article_id")))
    except Exception:
        pass
    for source in [decision.get("backup_article_ids", []), issue.get("candidate_article_ids", []), issue.get("article_ids", [])]:
        if not isinstance(source, list):
            continue
        for art_id in source:
            try:
                art_id = int(art_id)
            except Exception:
                continue
...
def v35_bodyless_article_score(article, issue, decision, requested_cat):
    text = v35_text(article)
    score = 0.0
    try:
        score += v20_article_pre_rep_score(article)
    except Exception:
        score += v20_float(article.get("랭킹점수"), 0)
    score += press_score(article.get("언론사", "")) * 2
    score += v20_float(issue.get("issue_score"), 0) * 0.25
    score += v20_int(decision.get("priority"), default=issue.get("max_ceo_priority", 3)) * 8
    if int(article.get("id", -1)) == int(decision.get("best_article_id") or -2):
        score += 20
    if v35_is_weak_offtopic(text):
        score -= 200
    if requested_cat == JSON_KEYS_ORDER[2] and not v35_is_competitor_strategy_or_global(text):
        score -= 160
    if requested_cat == JSON_KEYS_ORDER[1] and not v35_is_government_eligible(text) and v35_is_competitor_strategy_or_global(text):
        score += 30
    if article.get("Gemini제외") == "Y" or article.get("GeminiPR성") == "Y":
        score -= 200
    return score


def v35_should_allow_bodyless(decision, issue, requested_cat):
    if not issue or issue.get("exclude") or issue.get("is_pr"):
        return False
    text = v35_text(issue)
    if v35_is_weak_offtopic(text):
        return False
    priority = max(v20_int(decision.get("priority"), default=1), v20_int(issue.get("max_ceo_priority"), default=1), v20_int(issue.get("max_pa_priority"), default=1))
    issue_score = v20_float(issue.get("issue_score"), 0)
    selected_like = decision.get("include") is True or decision.get("v26_1_pool_source") == "selected" or decision.get("selection_source") in {"gemini", "editor", "selected"}
    high_value = priority >= V35_BODYLESS_MIN_PRIORITY or issue_score >= V35_BODYLESS_MIN_ISSUE_SCORE
    if not (selected_like or high_value):
        return False
    # Section-specific sanity check.
    preferred = v35_preferred_category_for_text(text, fallback=requested_cat)
    if requested_cat == JSON_KEYS_ORDER[2] and not v35_is_competitor_strategy_or_global(text):
        return False
    if requested_cat == JSON_KEYS_ORDER[1] and preferred == JSON_KEYS_ORDER[2]:
        return True
    if requested_cat == JSON_KEYS_ORDER[3] and preferred in {JSON_KEYS_ORDER[1], JSON_KEYS_ORDER[2]}:
        return True
    return True


def v35_create_title_link_only_item(decision, issue, article_by_id, requested_cat, order_counter):
    ids = v35_candidate_ids(decision, issue)
    candidates = []
    for art_id in ids:
        article = article_by_id.get(int(art_id)) if isinstance(article_by_id, dict) else None
        if not article:
            continue
        candidates.append((v35_bodyless_article_score(article, issue, decision, requested_cat), article))
    if not candidates:
        return None
    candidates.sort(key=lambda x: x[0], reverse=True)
    best_score, article_info = candidates[0]
    if best_score < -50:
        return None
    preferred_cat = v35_preferred_category_for_text(v35_text(article_info) + " " + v35_text(issue), fallback=requested_cat)
    # Keep user-selected section if it is sane; otherwise repair.
    if requested_cat == JSON_KEYS_ORDER[2] and not v35_is_competitor_strategy_or_global(v35_text(article_info) + " " + v35_text(issue)):
        requested_cat = preferred_cat
    elif requested_cat == JSON_KEYS_ORDER[1] and preferred_cat == JSON_KEYS_ORDER[2]:
        requested_cat = preferred_cat
    elif requested_cat == JSON_KEYS_ORDER[3] and preferred_cat in {JSON_KEYS_ORDER[1], JSON_KEYS_ORDER[2]}:
        requested_cat = preferred_cat
    ...
    try:
        item = v20_create_bodyless_report_item(
            ...
            decision,
            requested_cat,
            reason="all_candidates_failed_title_link_only",
        )
    except Exception:
        original_link = article_info.get("링크", "")
        real_url = normalize_url(original_link)
        item = {
            "카테고리": requested_cat,
            "카테고리명": JSON_KEY_TO_DISPLAY.get(requested_cat, requested_cat),
            "검색어": article_info.get("검색어", ""),
            "기사제목": article_info.get("기사제목", ""),
            "언론사": article_info.get("언론사", "") or guess_press_name_from_url(real_url),
            "게시일": article_info.get("게시일", ""),
            ...
            "본문전문": "",
            "본문글자수": 0,
            "본문추출방식": "title_link_only",
            "본문품질점수": 0,
            "본문품질사유": "all_candidates_failed_title_link_only",
            "본문사용가능": False,
            "본문상태": "본문추출실패_대체없음_제목링크만",
            "원래RSS링크": original_link,
            "링크": real_url,
            "랭킹점수": article_info.get("랭킹점수", ""),
        }
    item["본문전문"] = ""
    item["본문글자수"] = 0
    item["본문추출방식"] = "title_link_only"
    item["본문품질사유"] = "all_candidates_failed_title_link_only"
    item["본문상태"] = "본문추출실패_대체없음_제목링크만"
    item["선정단계"] = "title_link_only_fallback"
    item["선정순서"] = order_counter
    item["v35_title_link_only"] = "Y"
    item["v20_issue_id"] = issue.get("issue_id")
    item["v20_issue_group"] = issue.get("issue_group")
    item["v20_issue_family"] = issue.get("issue_family")
    item["v20_internal_category"] = issue.get("internal_category")
    item["대표선택점수"] = max(v20_float(item.get("대표선택점수"), -5), round(best_score, 2))
    # Copy Gemini diagnostic fields when available.
    for src, dst in [
        ("Gemini내부카테고리", "Gemini내부카테고리"),
        ("Gemini이슈패밀리", "Gemini이슈유형"),
        ("Gemini이슈그룹", "Gemini이슈그룹"),
        ("Gemini관련성", "Gemini관련성"),
        ("Gemini중요도", "Gemini중요도"),
        ("Gemini대외협력중요도", "Gemini대외협력중요도"),
        ("Gemini관련도", "Gemini관련도"),
        ("Gemini판단사유", "Gemini판단사유"),
    ]:
        if article_info.get(src) and not item.get(dst):
            item[dst] = article_info.get(src)
    item["Gemini카테고리"] = requested_cat
    item["Gemini선정사유"] = decision.get("reason", issue.get("label_reasons", ""))
    v35_set_category(item, requested_cat, "v35_title_link_only_category")
    return item


_BASE_v20_process_issue_v35 = v20_process_issue

def v20_process_issue(decision, issue_by_id, article_by_id, recent_past_items, final_report_data, body_failed_rows, skip_rows, processed_article_ids, order_counter):
    issue = issue_by_id.get(decision.get("issue_id"), {}) if isinstance(issue_by_id, dict) else {}
    if issue:
        requested = v19_normalize_category_key(decision.get("category"), fallback=v20_issue_to_output_category(issue))
        text = clean_html_text(" ".join([v35_text(issue), str(decision.get("category", "")), str(decision.get("reason", ""))]))
        preferred = v35_preferred_category_for_text(text, fallback=requested)
        if preferred != requested:
            decision = dict(decision)
            decision["category"] = preferred
            decision["v35_category_repair_before_process"] = f"{requested}_to_{preferred}"
    else:
        requested = v19_normalize_category_key(decision.get("category"), fallback="산업동향")

    item, new_order_counter, status = _BASE_v20_process_issue_v35(
        decision,
        issue_by_id,
        article_by_id,
        recent_past_items,
    ...
    if item:
        # Re-check section immediately after processing.
        old_cat = item.get("카테고리")
        preferred = v35_preferred_category_for_text(v35_text(item), fallback=old_cat)
        if preferred != old_cat:
            v35_set_category(item, preferred, f"v35_post_process_section_repair:{old_cat}_to_{preferred}")
        v35_repair_final_sections(final_report_data, skip_rows)
        return item, new_order_counter, status

    if status == "all_candidates_failed" and issue:
        requested = v19_normalize_category_key(decision.get("category"), fallback=v20_issue_to_output_category(issue))
        if v35_should_allow_bodyless(decision, issue, requested):
            fallback_item = v35_create_title_link_only_item(decision, issue, article_by_id, requested, order_counter)
            if fallback_item:
                dup_item, dup_reason = final_duplicate_reason(fallback_item, final_report_data)
                if dup_item:
                    add_duplicate_skip_row(skip_rows, fallback_item, dup_item, f"title_link_only_duplicate:{dup_reason}", stage="title_link_only_fallback")
                    return None, order_counter, "title_link_only_duplicate"
                tag_limit_reason = event_tag_limit_reason(fallback_item, final_report_data)
                if tag_limit_reason:
                    add_duplicate_skip_row(skip_rows, fallback_item, {"기사제목": "event_tag_cap", "링크": "", "대표선택점수": ""}, tag_limit_reason, stage="title_link_only_fallback")
                    return None, order_counter, "title_link_only_event_limited"
                final_report_data.append(fallback_item)
                v35_repair_final_sections(final_report_data, skip_rows)
                return fallback_item, order_counter + 1, "title_link_only_fallback"
    return item, new_order_counter, status


# Refill gating: do not allow weak/off-topic issues to enter merely to hit total count.
_BASE_v26_1_is_good_refill_candidate_v35 = v26_1_is_good_refill_candidate

def v26_1_is_good_refill_candidate(decision, issue, cat=None):
    cat = cat or v26_1_decision_category(decision, issue)
    try:
        if not _BASE_v26_1_is_good_refill_candidate_v35(decision, issue, cat=cat):
            return False
    except TypeError:
        if not _BASE_v26_1_is_good_refill_candidate_v35(decision, issue):
            return False
    text = clean_html_text(" ".join([v35_text(issue), str(decision.get("category", "")), str(decision.get("reason", ""))]))
    if v35_is_weak_offtopic(text):
        return False
    preferred = v35_preferred_category_for_text(text, fallback=cat)
    # Refill cannot force a candidate into the wrong section.
    if cat == JSON_KEYS_ORDER[2]:
        return v35_is_competitor_strategy_or_global(text)
    if cat == JSON_KEYS_ORDER[1]:
        return v35_is_government_eligible(text) and not v35_is_specific_competitor_event(text)
    if cat == JSON_KEYS_ORDER[3]:
        return v35_is_industry_eligible(text)
    return preferred == cat or cat == JSON_KEYS_ORDER[0]


# Competitor portfolio eligibility also uses the stricter v35 gate.
try:
    _BASE_v32_competitor_issue_eligible_v35 = v32_competitor_issue_eligible
    def v32_competitor_issue_eligible(issue):
        text = v35_text(issue)
        if not v35_is_competitor_strategy_or_global(text):
            issue["v35_competitor_block_reason"] = "not_core_competitor_strategy_or_risk"
            return False
        return _BASE_v32_competitor_issue_eligible_v35(issue)
except Exception:
    pass

try:
    _BASE_v32_competitor_article_eligible_v35 = v32_competitor_article_eligible
    def v32_competitor_article_eligible(article):
        text = v35_text(article)
        if not v35_is_competitor_strategy_or_global(text):
            article["v35_competitor_block_reason"] = "not_core_competitor_strategy_or_risk"
            return False
        return _BASE_v32_competitor_article_eligible_v35(article)
except Exception:
    pass

try:
    _BASE_v30_competitor_eligible_v35 = v29_competitor_eligible
    def v29_competitor_eligible(item):
        text = v35_text(item)
        if not v35_is_competitor_strategy_or_global(text):
            return False, "v35_not_core_competitor_strategy_or_risk"
        return _BASE_v30_competitor_eligible_v35(item)
except Exception:
    pass


# Repair wrappers used by main/refill/QA.
try:
    _BASE_v34_repair_competitor_government_leaks_v35 = v34_repair_competitor_government_leaks
    def v34_repair_competitor_government_leaks(final_report_data, skip_rows=None):
        try:
            _BASE_v34_repair_competitor_government_leaks_v35(final_report_data, skip_rows)
        except Exception:
            pass
        return v35_repair_final_sections(final_report_data, skip_rows)
except Exception:
    pass

try:
    _BASE_v32_repair_final_items_v35 = v32_repair_final_items
    def v32_repair_final_items(final_report_data, skip_rows=None):
        try:
            _BASE_v32_repair_final_items_v35(final_report_data, skip_rows)
        except Exception:
            pass
        return v35_repair_final_sections(final_report_data, skip_rows)
except Exception:
...
try:
    _BASE_v33_repair_final_items_v35 = v33_repair_final_items
    def v33_repair_final_items(final_report_data, skip_rows=None):
        try:
            _BASE_v33_repair_final_items_v35(final_report_data, skip_rows)
        except Exception:
            pass
        return v35_repair_final_sections(final_report_data, skip_rows)
except Exception:
...
_BASE_v26_1_refill_after_final_repair_v35 = v26_1_refill_after_final_repair

def v26_1_refill_after_final_repair(decisions, backup_decisions, issues, issue_by_id, article_by_id, recent_past_items, final_report_data, body_failed_rows, skip_rows, processed_article_ids, order_counter, status_counts):
    order_counter = _BASE_v26_1_refill_after_final_repair_v35(
        decisions=decisions,
        backup_decisions=backup_decisions,
        issues=issues,
        issue_by_id=issue_by_id,
...
# Keep title/link-only summaries minimal, regardless of RSS stub.
_BASE_v20_bodyless_summary_v35 = v20_bodyless_summary

def v20_bodyless_summary(item):
    return "원문 본문 자동 추출이 제한돼 세부 내용은 링크 확인이 필요함."


_BASE_build_structured_briefing_v35 = build_structured_briefing

def build_structured_briefing(final_report_data, summary_map=None):
    summary_map = dict(summary_map or {})
    for item in final_report_data:
        status = str(item.get("본문상태", ""))
        if status.startswith("본문추출실패") or item.get("v35_title_link_only") == "Y":
            bid = str(item.get("브리핑ID", ""))
            if bid:
                summary_map[bid] = v20_bodyless_summary(item)
    return _BASE_build_structured_briefing_v35(final_report_data, summary_map)


_BASE_v20_gemini_quality_check_v35 = v20_gemini_quality_check

def v20_gemini_quality_check(client, final_report_data, final_briefing_text):
    try:
        skip_like = []
        v35_repair_final_sections(final_report_data, skip_like)
    except Exception:
        pass
    qa = _BASE_v20_gemini_quality_check_v35(client, final_report_data, final_briefing_text)
    try:
        data = v22_parse_qa_json(qa) if qa else {"overall": "양호", "warnings": [], "suggested_human_checks": []}
    except Exception:
        return qa
    warnings = data.get("warnings") if isinstance(data.get("warnings"), list) else []
    checks = data.get("suggested_human_checks") if isinstance(data.get("suggested_human_checks"), list) else []
    weak_comp = [x.get("기사제목", "") for x in final_report_data if x.get("카테고리") == JSON_KEYS_ORDER[2] and not v35_is_competitor_strategy_or_global(v35_text(x))]
    if weak_comp:
        msg = "경쟁사/해외 섹션에 v35 기준 약한 후보가 남아 있음: " + "; ".join(t[:60] for t in weak_comp[:3])
        warnings.append(msg)
        checks.append(msg)
        data["overall"] = "문제"
    bodyless_count = sum(1 for x in final_report_data if str(x.get("본문상태", "")).startswith("본문추출실패"))
    if bodyless_count:
        checks.append(f"본문 추출 실패로 제목·언론사·링크만 포함한 기사 {bodyless_count}건은 원문 확인 필요")
        if data.get("overall") == "양호":
            data["overall"] = "주의"
    data["warnings"] = warnings
    data["suggested_human_checks"] = checks
