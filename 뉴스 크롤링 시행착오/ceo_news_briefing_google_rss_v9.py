...
# NOTE (v9): v7 is_report_item_relevant는 위와 같이 bypass 없이 유지.
# v8에서 is_v8_kakao_gr_critical() bypass를 넣었지만, 관련성은 통과해도
# 당일 중복 제거(final_duplicate_reason)는 반드시 적용해야 하므로 여기서는 제거함.
...
V6_VERSION = "google_rss_v9_cpo_dedup_fix"
...
def is_report_item_relevant(report_item, json_key):
    ...
    if json_key == "자사_및_계열사_이슈":
        # v9 수정: is_v8_kakao_gr_critical() bypass 제거.
        # 고위험 기사도 관련성 필터는 통과시키되, 당일 중복(final_duplicate_reason)은
        # 반드시 적용해야 하므로 여기서 early-return하지 않음.
        if not SELF_KAKAO_PATTERN.search(text):
            return False, "self_category_without_kakao_entity"
        if keyword == "카카오톡" and not re.search(r"카카오톡|카톡|카카오", title, flags=re.IGNORECASE):
            kakao_mentions = len(re.findall(r"카카오|카톡|Kakao", text, flags=re.IGNORECASE))
...
def final_duplicate_reason(new_item, existing_items):
    """v9: executive_change(CPO/임원 퇴사 등) 당일 중복 처리 추가.
    같은 날 같은 인물/사건의 퇴사·사의 기사는 대표 1건만 남김."""
    new_title = new_item.get("기사제목", "")
    new_body = new_item.get("본문전문", "")[:1800]
    new_text = f"{new_title} {new_body}"
    new_tokens = tokenize_for_similarity(new_text)
    ...
    for old in existing_items:
        ...
        # v9 추가: executive_change(CPO/임원 퇴사·사의) 당일 중복.
        # "CPO 퇴사한다" / "CPO 결국 사의 표명" / "CPO 퇴장" 처럼 표현이 달라도
        # 같은 인물(홍민택 등) + 같은 사건 태그이면 대표 1건만 허용.
        if "executive_change" in new_tags and "executive_change" in old_tags:
            # 주요 인물이 겹치거나 카카오 공통 엔티티를 공유하면 같은 사건으로 판단
            exec_person_match = bool(re.search(r"홍민택|CPO", new_text) and re.search(r"홍민택|CPO", old_text))
            if exec_person_match and (token_score >= 0.08 or title_seq >= 0.38 or body_ngram_score >= 0.14):
                return old, f"same_current_executive_change:{title_seq:.2f}"
            # 인물 명시 없어도 카카오 엔티티 + 퇴사/사의 키워드 공유 시
            if shared_entities and re.search(r"퇴사|사의|사퇴|사임", new_text) and re.search(r"퇴사|사의|사퇴|사임", old_text):
                if token_score >= 0.12 or title_seq >= 0.45:
                    return old, f"same_current_executive_change_generic:{title_seq:.2f}"
...
# ==========================================
# v8 overrides: GR/public-affairs priority + flexible quotas
# ==========================================

# 버전명은 기존 변수명을 재사용합니다.
V6_VERSION = "google_rss_v9_cpo_dedup_fix"

# 카테고리 유연 쿼터. CATEGORY_TARGET은 "채우고 싶은 기본선"이고, CATEGORY_MAX는 하드캡입니다.
CATEGORY_MIN = {
    "자사_및_계열사_이슈": 3,
    "정부_국회": 5,
    "경쟁사_해외이슈": 3,
...
# 신규 사건 태그: v7에서 약했던 임원 이탈/노동부 진정/최저임금법 이슈를 별도 고위험 이슈로 분리.
EVENT_PATTERNS.update({
    "executive_change": r"홍민택|CPO|CTO|CFO|C레벨|임원|경영진|성과리더|대표.*(퇴사|사퇴|사임|내정|선임|교체)|퇴사|사퇴|사임|조직개편|인사",
    "labor_law_complaint": r"최저임금법|노동부\s*진정|고용노동부|노동부|근로감독|임금체불|민주노총|화섬식품노조|플랫폼\s*노동|대리운전.*최저임금|노동청",
    "service_reform_backlash": r"카톡\s*개편|카카오톡\s*개편|친구탭|업데이트\s*논란|UX\s*논란|이용자\s*반발|받글|프로필.*스토리",
})
STRONG_EVENT_TAGS.update({"executive_change", "labor_law_complaint", "service_reform_backlash"})

# 특정 사건 반복 제한. 전역 제한이 아니라 event_limit_bucket_v8에서 주체/세부 버킷별로 적용됩니다.
# v9 수정: executive_change 2→1. final_duplicate_reason에서 당일 중복을 잡아주므로
# 상한도 1건으로 줄여 같은 임원 퇴사 기사가 2건 이상 최종 보고서에 들어가지 않도록 함.
MAX_FINAL_PER_EVENT_TAG.update({
    "executive_change": 1,
    "labor_law_complaint": 2,
    "service_reform_backlash": 2,
})

V8_CRITICAL_PATTERN = re.compile(
    r"홍민택|CPO|카톡\s*개편|카카오톡\s*개편|개편\s*논란|퇴사|사퇴|사임|임원|경영진|조직개편|"
    r"최저임금법|노동부\s*진정|고용노동부|근로감독|민주노총|임금체불|대리운전.*최저임금|"
    r"압수수색|수사\s*착수|입건|기소|구형|판결|선고|제재|과징금|행정소송|고발|"
    r"조정\s*결렬|조정\s*중지|조정\s*회의|쟁의권|파업|총파업|임단협|성과급|RSU|"
    r"개인정보|유출|해킹|침해사고|피싱|악성코드|장애|먹통|매각|인수|합병|상장|IPO|지분|처분|최대주주",
    re.IGNORECASE,
)

V8_STRICT_EXCLUDE_TITLE_PATTERNS = V6_STRICT_EXCLUDE_TITLE_PATTERNS + [
    r"네이버\s*블로그$", r":\s*네이버\s*블로그", r"Naver\s*Blog",
    r"개별\s*사건", r"인출책\s*징역", r"피해자\s*생\s*마감",
]


def is_v8_kakao_gr_critical(text):
    """카카오 대외협력/대관 관점에서 놓치면 안 되는 고위험/고중요 이슈."""
    text = clean_html_text(text)
    has_kakao = bool(SELF_KAKAO_PATTERN.search(text))
    if not has_kakao:
        return False
    return bool(re.search(
        r"홍민택|CPO|카톡\s*개편|카카오톡\s*개편|개편\s*논란|퇴사|사퇴|사임|임원|경영진|조직개편|"
        r"최저임금법|노동부\s*진정|고용노동부|근로감독|민주노총|임금체불|대리운전.*최저임금|"
        r"파업|총파업|쟁의권|조정|지노위|노동위|성과급|RSU|고용불안|근로기준법|"
        r"개인정보|수사|소송|과징금|행정소송|장애|먹통|매각|지분|경영권",
        text,
        flags=re.IGNORECASE,
    ))


def is_individual_crime_case(title, body=""):
    text = f"{clean_html_text(title)} {clean_html_text(body)[:800]}"
    if not re.search(r"보이스피싱|피싱|사기|인출책|수거책", text):
        return False
    # 정책/제도/기업 대응이면 살립니다.
    if re.search(r"정부|금융위|금감원|방미통위|과기정통부|경찰청|대책|종합대책|제도|법안|시행|감소|차단|대응|플랫폼|통신사|은행|금융회사", text):
        return False
    if re.search(r"징역|선고|피해자|생\s*마감|사망|구속|송치|인출책|수거책|범행", text):
        return True
    return False


def article_importance_score(title, body="", json_key="", keyword=""):
    """v8: 키워드가 아니라 카카오 대외 리스크/정책 리스크 관점의 중요도를 더 강하게 반영."""
    title_clean = clean_html_text(title)
    body_clean = clean_html_text(body)
    text = f"{title_clean} {body_clean[:2400]}"
    tags = detect_event_tags(text)
    entities = detect_entities(text)
    score = 0.0
    ...
    if V8_CRITICAL_PATTERN.search(text):
        score += 10

    # 카카오 대관/대외협력 담당자가 특히 봐야 하는 자사 리스크.
    if json_key == "자사_및_계열사_이슈":
        score += 8
        if is_v8_kakao_gr_critical(text):
            score += 24
        if re.search(r"\[단독\]|단독", title_clean) and SELF_KAKAO_PATTERN.search(text):
            score += 8
        if re.search(r"홍민택|CPO|카톡\s*개편|카카오톡\s*개편|개편\s*논란|퇴사|사퇴|사임", text, flags=re.IGNORECASE):
            score += 16
        if re.search(r"최저임금법|노동부\s*진정|고용노동부|근로감독|민주노총|임금체불", text, flags=re.IGNORECASE):
            score += 16
    elif json_key == "정부_국회":
        score += 7
        if re.search(r"공정위|금융위|금감원|과기정통부|방미통위|개보위|국회|법안|시행령|입법|과징금|조사국|플랫폼", text):
            score += 7
    elif json_key == "경쟁사_해외이슈":
        score += 4
        if re.search(r"네이버|구글|오픈AI|MS|메타|애플|쿠팡|토스|배달의민족|배민|SKT|KT|LG유플러스", text, flags=re.IGNORECASE):
            score += 4
    elif json_key == "산업동향":
        score += 2

    for entity in ["카카오", "카카오페이", "카카오뱅크", "카카오모빌리티", "카카오게임즈", "카카오엔터", "네이버", "구글", "오픈AI", "공정위", "금융위", "금감원", "과기정통부", "방미통위", "고용노동부"]:
        if entity in entities or entity in text:
            score += 2.5

    if is_individual_crime_case(title_clean, body_clean):
        score -= 28

    for pattern in LOW_VALUE_TITLE_PATTERNS:
        if re.search(pattern, title_clean, flags=re.IGNORECASE):
            score -= 15
    if PHOTO_OR_CAPTION_TITLE_RE.search(title_clean):
        score -= 30
...
def is_article_obviously_low_value(article):
    ...
    # 카카오 고위험 이슈는 일반 저가치 패턴보다 우선 살림.
    if is_v8_kakao_gr_critical(text):
        return False, ""

    for pattern in V8_STRICT_EXCLUDE_TITLE_PATTERNS:
        if re.search(pattern, title, flags=re.IGNORECASE):
            return True, f"strict_low_value_title:{pattern}"
    ...
    if is_individual_crime_case(title, summary):
...
def is_report_item_relevant(report_item, json_key):
    ...
    # v9 수정: is_v8_kakao_gr_critical() bypass 완전 제거.
    # 고위험 기사라도 당일 중복(final_duplicate_reason)은 반드시 통과해야 하므로
    # 여기서 early-return하지 않음. 관련성(SELF_KAKAO_PATTERN)은 아래에서 정상 검사.

    if re.search(r"blog\.naver\.com|m\.blog\.naver\.com", link, flags=re.IGNORECASE):
        return False, "naver_blog_not_primary_source"
    ...
    if is_individual_crime_case(title, body):
        return False, "individual_crime_case_not_policy"

    low, reason = is_article_obviously_low_value({
        "기사제목": title, "본문요약": body[:700], "검색어": keyword, "언론사": source
    })
    if low:
        return False, reason
    ...
    if json_key == "경쟁사_해외이슈":
        if re.search(r"케이스\s*유출|렌더링|출시\s*예상|스펙\s*유출|화면\s*없는\s*기기", title):
            return False, "consumer_product_rumor"
        brand_pat = BRAND_KEYWORD_PATTERNS.get(keyword)
        if brand_pat and not brand_pat.search(text[:1000]):
            return False, "competitor_brand_absent_from_front_text"

    if json_key == "산업동향":
        if not re.search(r"AI|인공지능|데이터센터|GPU|NPU|클라우드|플랫폼|망\s*사용료|스테이블코인|디지털자산|보안|해킹|딥페이크|저작권|알고리즘|에이전트", text, flags=re.IGNORECASE):
            return False, "industry_without_core_digital_trend"
        if re.search(r"세라믹기술원|장애인\s*표준사업장|교통사고", title):
            return False, "generic_industry_or_local_news"
...
def article_similarity(candidate, past_item, mode="body"):
    """v9: high_update 임계값 완화(0.86→0.72). CPO 퇴사 등 같은 사건을 다른 언론사가
    다르게 표현해도 당일 중복으로 걸러지도록 수정."""
    candidate_title = candidate.get("기사제목") or candidate.get("title") or ""
    candidate_summary = candidate.get("본문요약") or candidate.get("summary") or candidate.get("본문전문") or ""
    candidate_link = normalize_url(candidate.get("링크") or candidate.get("link") or "")
    ...
    if candidate_link and past_link and candidate_link == past_link:
        return {"is_duplicate": True, "reason": "same_url", "score": 1.0, "title_score": 1.0, "token_score": 1.0, "text_score": 1.0, "shared_entities": "", "shared_tags": ""}

    candidate_title_norm = normalize_for_similarity(candidate_title)
    past_title_norm = normalize_for_similarity(past_title)
    if candidate_title_norm and past_title_norm and candidate_title_norm == past_title_norm:
        return {"is_duplicate": True, "reason": "same_title", "score": 1.0, "title_score": 1.0, "token_score": 1.0, "text_score": 1.0, "shared_entities": "", "shared_tags": ""}
    ...
    # v9 수정: high_update 임계값을 0.86→0.72 / 0.34→0.22로 완화.
    # 원래 0.86은 제목이 거의 동일해야만 중복 판정해서 "CPO 퇴사한다" vs "CPO 결국 사의 표명"
    # 같이 표현만 다른 같은 사건이 통과됐음. 0.72로 낮추면 이 케이스가 잡힘.
    high_update = is_v8_kakao_gr_critical(candidate_text) and re.search(
        r"홍민택|CPO|퇴사|사퇴|사임|최저임금법|노동부\s*진정|고용노동부|근로감독|민주노총", candidate_text, flags=re.IGNORECASE)
    if high_update:
        is_dup = title_score >= 0.72 and token_score >= 0.22
        return {
            "is_duplicate": bool(is_dup),
            "reason": "high_update_near_same_title" if is_dup else "high_update_not_duplicate",
            "score": round(combined, 4),
            "title_score": round(title_score, 4),
            "token_score": round(token_score, 4),
            "text_score": round(text_score, 4),
    ...
    is_duplicate = False
    reason = ""
    if should_cluster_as_duplicate(mode, title_score, token_score, text_score, shared_entities, shared_tags, candidate_text, past_text):
        is_duplicate = True
        reason = f"same_issue_signature:{extract_event_stage(candidate_text, candidate_tags)}"
    elif title_score >= 0.70 and token_score >= 0.18:
        is_duplicate = True
...
def labor_bucket_v8(item):
    text = f"{item.get('기사제목','')} {item.get('본문전문','')[:1400]} {item.get('RSS요약','')}"
    if re.search(r"최저임금법|노동부\s*진정|고용노동부|근로감독|민주노총|임금체불|대리운전.*최저임금", text, flags=re.IGNORECASE):
        return "labor_law_complaint"
    if re.search(r"성과급|RSU|영업이익.*성과급|임금|임단협", text):
        return "labor_pay_dispute"
    if re.search(r"조정|지노위|노동위|쟁의권|파업|총파업|결의대회|찬반투표", text):
        return "labor_strike_negotiation"
    if is_kakao_labor_issue(item):
        return "labor_general"
    return ""


def event_limit_bucket_v7(item, tag):
    text = f"{item.get('기사제목','')} {item.get('본문전문','')[:1200]}"
    entities = detect_entities(text)
    entity_order = [
        "카카오게임즈", "카카오페이", "카카오뱅크", "카카오모빌리티", "카카오엔터", "카카오", 
        "쿠팡", "네이버", "구글", "오픈AI", "SKT", "KT", "LGU+", "토스", "배달의민족",
        "공정위", "금융위", "금감원", "과기정통부", "방미통위", "고용노동부",
    ]
    primary = "general"
    for ent in entity_order:
        if ent in entities or ent in text:
            primary = ent
            break
    if tag in {"labor_dispute", "labor_law_complaint"}:
        return f"{tag}:{primary}:{labor_bucket_v8(item) or item.get('사건단계') or 'general'}"
    if tag in {"executive_change", "service_reform_backlash"}:
        return f"{tag}:{primary}"
    stage = item.get("사건단계") or ""
    if tag == "earnings":
        return f"{tag}:{primary}"
...
def event_tag_limit_reason(new_item, existing_items):
    """v8: 카카오 노사 전체를 한 덩어리로 막지 않고, 법적 진정/성과급/조정 단계를 나눠 허용."""
    if is_kakao_labor_issue(new_item):
        new_bucket = labor_bucket_v8(new_item)
        bucket_limit = {
            "labor_law_complaint": 1,
            "labor_pay_dispute": 2,
            "labor_strike_negotiation": 2,
            "labor_general": 1,
        }.get(new_bucket, 1)
        same_bucket_count = sum(1 for old in existing_items if labor_bucket_v8(old) == new_bucket)
        total_labor_count = sum(1 for old in existing_items if is_kakao_labor_issue(old))
        if same_bucket_count >= bucket_limit:
            return f"event_topic_limit:kakao_{new_bucket}:{bucket_limit}"
        if total_labor_count >= 4:
            return "event_topic_limit:kakao_labor_total:4"
...
def enforce_selection_limits(selection, ranked_candidates):
    """v8: Gemini가 고른 기사 중 고중요 기사는 카테고리 최대치까지 허용하되, 최소치 미달은 랭킹으로 보충."""
    result = {key: [] for key in JSON_KEYS_ORDER}
    used = set()
    ...
    # 하드 총량 초과 시 산업동향→경쟁사→정부→자사 순으로 낮은 랭킹 후보 제거
    id_to_rank = {int(a["id"]): float(a.get("랭킹점수") or 0) for a in ranked_candidates}
    while sum(len(v) for v in result.values()) > MAX_SELECT_COUNT:
        removable = []
        for key in reversed(JSON_KEYS_ORDER):
            for art_id in result[key]:
                # 카테고리 최소치 아래로는 줄이지 않음
                if len(result[key]) <= CATEGORY_MIN.get(key, 0):
                    continue
                removable.append((id_to_rank.get(int(art_id), 0), key, art_id))
        if not removable:
            break
        _, key, art_id = min(removable, key=lambda x: x[0])
...
def deterministic_selection(ranked_candidates):
    ...
    for json_key in JSON_KEYS_ORDER:
        ...
        for article in category_candidates[:target]:
            art_id = int(article["id"])
            result[json_key].append(art_id)
            used_ids.add(art_id)
...
def main():
    ...
    print("\n🧠 [STEP 2] 후보 랭킹 + Gemini가 과거 7일 중복 이슈를 제외하고 핵심 기사를 선별합니다...")
    print(f"   └ RSS 전체 {len(raw_articles)}개 중 랭킹 상위 {len(ranked_candidates)}개만 Gemini 후보로 전달합니다.")
    print("   └ v8: 카카오 대관/대외협력 관점의 중요도, 유연 쿼터, 대표기사 선별을 적용합니다.")
...
"""
    ...
    print("\n🕵️‍♂️ [STEP 3] 선별/보충 후보만 원문 URL 변환 후 본문 전문을 추출합니다...")
    print("   └ v8: 짧아도 고위험/고중요 이슈는 통과시키고, 오래된 원문·오탐·단순 사건은 제외합니다.")
    ...
    # 대표 기사 점수 기준으로 카테고리 내 정렬하되, 카테고리 순서는 유지
    ordered_final = []
    for json_key in JSON_KEYS_ORDER:
        items = [x for x in final_report_data if x.get("카테고리") == json_key]
        items = sorted(items, key=lambda x: (float(x.get("랭킹점수") or 0), float(x.get("대표선택점수") or 0)), reverse=True)
