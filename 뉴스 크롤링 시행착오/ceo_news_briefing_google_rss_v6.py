...
def collect_with_google_rss(recent_past_items):
    ...
    print("   └ v5: URL뿐 아니라 제목+언론사 canonical dedupe, 검색 오탐 필터, shard overlap 조기중단을 적용합니다.")
...
V6_VERSION = "google_rss_v6_story_cluster_representative"
...
def main():
    ...
    print("\n🧠 [STEP 2] 후보 랭킹 + Gemini가 과거 7일 중복 이슈를 제외하고 핵심 기사를 선별합니다...")
    print(f"   └ RSS 전체 {len(raw_articles)}개 중 랭킹 상위 {len(ranked_candidates)}개만 Gemini 후보로 전달합니다.")
    print("   └ v6: 중복 기사는 본문 추출 후 대표선택점수로 더 좋은 기사 1개를 남깁니다.")
...
1. 최근 7일 과거 보고서에 이미 정리된 사건과 실질적으로 같은 내용이면 제외해.
2. 단순 후속 기사라도 새 사실, 새 수치, 새 조치, 새 당국 발표가 없으면 제외해.
3. 단, 1차 조정→2차 조정, 투표→쟁의권, 소송 제기→판결처럼 사건 단계가 바뀐 경우는 업데이트 기사로 볼 수 있어.
4. 오피니언, 사설, 칼럼, 전문가 기고는 제외해.
5. 사진/포토/캡션 기사, 외국어 저품질 기사, 제품 케이스 유출/렌더링 루머는 제외해.
6. 동일 사건에 대해 여러 기사가 있으면 일단 대표로 보이는 기사 1개만 선택해. 최종 대표 기사는 코드가 본문 품질로 다시 고를 거야.
7. 카카오 및 계열사는 과징금, 경영진, 서비스 장애, 지배구조, 실적, 규제, 수사, 소송, 대형 제휴, 노사 등 무거운 내용 위주로 골라.
8. 타사 이름만 들어간 무관한 기사는 제외해.
9. 총 13~15개를 골라.
10. 카테고리별 기준은 자사 및 계열사 3~4개, 정부/국회 4~5개, 경쟁사/해외이슈 3~4개, 산업동향 1~2개야.
...
"""
    ...
    print("\n🕵️‍♂️ [STEP 3] 선별/보충 후보만 원문 URL 변환 후 본문 전문을 추출합니다...")
    print("   └ v6: 본문 700자 미만이어도 중요도 높은 기사는 통과 가능하고, 중복 기사끼리는 대표선택점수로 교체합니다.")
