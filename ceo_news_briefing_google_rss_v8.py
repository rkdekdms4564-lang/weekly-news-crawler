...
def is_report_item_relevant(report_item, json_key):
    ...
    if is_v8_kakao_gr_critical(text) and json_key == "자사_및_계열사_이슈":
        return True, ""
...
def article_similarity(candidate, past_item, mode="body"):
    """v8: 고위험 업데이트성 기사(CPO 퇴사/노동부 진정 등)는 과거 큰 이슈와 느슨하게 묶어버리지 않음."""
    candidate_title = candidate.get("기사제목") or candidate.get("title") or ""
    candidate_summary = candidate.get("본문요약") or candidate.get("summary") or candidate.get("본문전문") or ""
    candidate_link = normalize_url(candidate.get("링크") or candidate.get("link") or "")
    ...
    # 고위험 새 단계는 거의 같은 제목이 아니면 past duplicate로 제거하지 않습니다.
    high_update = is_v8_kakao_gr_critical(candidate_text) and re.search(r"홍민택|CPO|퇴사|사퇴|사임|최저임금법|노동부\s*진정|고용노동부|근로감독|민주노총", candidate_text, flags=re.IGNORECASE)
    if high_update:
        is_dup = title_score >= 0.86 and token_score >= 0.34
        return {
